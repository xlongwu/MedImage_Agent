...
def build_default_go_no_go_review() -> DicomConversionGoNoGoReview:
    ...
    criteria: list[DicomConversionGoNoGoCriterion] = [
        ...
        DicomConversionGoNoGoCriterion(gate_id=12, label="Rollback policy defined", status="met", evidence="Phase 4J-0: dry-run, quarantine, delete modes implemented; rawdata + metadata protected; 14 tests passed", risk="Partial outputs", action_before_go="Done — rollback implemented"),
        ...
        DicomConversionGoNoGoCriterion(gate_id=27, label="Approval gate schema complete", status="met", evidence="17 preconditions defined + Phase 4J-1: gate evaluated before execution, blocks if not approved", risk="Missing checks", action_before_go="Gate integrated into execution"),
        DicomConversionGoNoGoCriterion(gate_id=28, label="Audit persists before exec", status="met", evidence="Phase 4J-1: audit_execution_start.json written before dcm2niix, audit_execution_final.json after; provenance references all evidence", risk="Race condition", action_before_go="Gate integration complete"),
    ...
    return DicomConversionGoNoGoReview(
        review_id="phase-4g-4-review",
        decision="CONDITIONAL_GO",
        reviewer="MedImage Agent Phase 4G-4 final automated review",
        ...
        criteria=criteria,
        blocking_issues=[],
        recommendation="FULL GO ELIGIBLE — REQUIRES FINAL HUMAN RELEASE APPROVAL. 32/32 gates met, zero missing, zero partial. All safety gates satisfied. Public conversion remains disabled by policy. Human sign-off required for public enablement.",
        next_step="Phase 4K-0: Release hardening before public conversion UI.",
    )
