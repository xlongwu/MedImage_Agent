"""ALFF/ReHo Dry-Run Schema — Phase 5L."""
from __future__ import annotations
from pydantic import BaseModel, Field


class AlffRehoDryRunRequest(BaseModel):
    registered_stage_output_id: str = ""; functional_input_dir: str = ""
    compute_alff: bool = True; compute_falff: bool = True; compute_reho: bool = True
    reho_neighbors: int = 27
...
class AlffRehoDryRunResponse(BaseModel):
    ...
    alff_plan_paths: list[str] = Field(default_factory=list)
    reho_plan_paths: list[str] = Field(default_factory=list)
...
def alff_reho_safety_flags() -> dict[str, bool]:
