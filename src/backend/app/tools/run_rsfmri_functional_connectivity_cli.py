...
def _make_approved(s: Path, t: Path) -> Path:
    import yaml; d = yaml.safe_load(s.read_text(encoding="utf-8"))
    ...
    t.parent.mkdir(parents=True, exist_ok=True); t.write_text(yaml.safe_dump(d, sort_keys=False), encoding="utf-8"); return t
def main() -> int:
    args = sys.argv[1:]; ap = "--approve" in args; args = [a for a in args if a != "--approve"]
    pc = Path(args[0]) if args else Path("examples/project_config_dataset.yaml")
    pl = Path(args[1]) if len(args) > 1 else Path("examples/pipeline_rsfmri_functional_connectivity.yaml")
    if ap: pl = _make_approved(pl, Path("outputs/work/rsfmri/approved_pipeline_fc.yaml"))
