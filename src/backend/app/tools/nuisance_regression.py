...
def run_python_nuisance_regression_subject(
    ...
    if not input_path.exists():
        return _write_failure(
            subject_id, result_json, qc_json, qc_md, [f"Input NIfTI not found: {input_path}"]
        )
    if not _safe_input_path(input_path, subject_id, derivatives_dir):
        return _write_failure(
            ...
            qc_json,
            qc_md,
            [f"Unsafe nuisance regression input: {input_path}"],
        )
    if not confounds_path.exists():
        return _write_failure(
            subject_id, result_json, qc_json, qc_md, [f"Confounds TSV not found: {confounds_path}"]
        )
    ...
    try:
        ...
        finite_fraction = (
            float(np.count_nonzero(finite_mask) / residual_4d.size) if residual_4d.size else 0.0
        )
...
def _write_failure(
    subject_id: str, result_json: Path, qc_json: Path, qc_md: Path, errors: list[str]
) -> dict[str, Any]:
    qc = {
        "ok": False,
        "node_id": "nuisance_regression_qc_subject",
...
def write_nuisance_regression_dataset_report(
    ...
    variance_ratios = [
        float(s["variance_ratio"]) for s in subjects if s.get("variance_ratio") is not None
    ]
    summary = {
    ...
    lines = [
        "# rs-fMRI Nuisance Regression QC Dataset Report",
        "",
        "## Summary",
        "",
        f"- Subjects total: {subjects_total}",
        f"- PASS: {pass_count}",
        ...
        "## Subjects",
        "",
        "| Subject | Status | Confounds | Rank | Variance Ratio |",
        "|---|---|---:|---:|---:|",
    ]
    for item in subjects:
        lines.append(
            f"| {item.get('subject_id')} | {item.get('regression_qc_status')} | {item.get('confound_columns')} | {item.get('confound_rank')} | {item.get('variance_ratio')} |"
        )
    lines += [
        "",
        "## Safety Note",
        "",
        "This report summarizes derivative nuisance regression QC outputs only. It does not modify rawdata.",
    ]
