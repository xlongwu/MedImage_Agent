...
def _make_approved_pipeline_copy(source: Path, target: Path) -> Path:
    try: import yaml
    except ImportError as exc: raise RuntimeError("Missing PyYAML.") from exc
...
def main() -> int:
    ...
    pl = Path(args[1]) if len(args) > 1 else Path("examples/pipeline_rsfmri_alff_falff.yaml")
    if approved: pl = _make_approved_pipeline_copy(pl, Path("outputs/work/rsfmri/approved_pipeline_alff_falff.yaml"))
    summary = run_pipeline(pc, pl)
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    s = summary.get("status"); return 0 if s == "SUCCESS" else (1 if s == "INVALID" else 2)
