...
def run_reho_subject(subject_id: str, derivatives_dir: str, backend: str = "python", neighborhood: int = 27, use_gm_mask: bool = False, input_nii: str | None = None, prefer_gpu: bool = True, require_gpu: bool = False) -> dict[str, Any]:
    if backend == "gpu":
        if not input_nii:
            return {"ok": False, "node_id": "reho_subject", "backend": backend, "subject_id": subject_id, "outputs": [], "warnings": [], "errors": ["GPU backend requires input_nii."]}
        r = run_gpu_reho_subject(subject_id=subject_id, input_nii=input_nii, derivatives_dir=derivatives_dir, neighborhood=neighborhood, use_gm_mask=use_gm_mask, prefer_gpu=prefer_gpu, require_gpu=require_gpu)
