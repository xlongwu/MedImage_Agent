...
from src.backend.app.tools.gpu_temporal_filtering_runner import run_temporal_filtering_subject as run_gpu_temporal_filtering
...
def run_temporal_filtering_subject(
    ...
    input_nii: str | None = None, prefer_gpu: bool = True, require_gpu: bool = False,
) -> dict[str, Any]:
    if backend == "gpu":
        if not input_nii:
            return {"ok": False, "node_id": "temporal_filtering_subject", "backend": backend, "subject_id": subject_id, "outputs": [], "warnings": [], "errors": ["GPU backend requires input_nii."]}
        r = run_gpu_temporal_filtering(subject_id=subject_id, input_nii=input_nii, derivatives_dir=derivatives_dir, tr=tr or fallback_tr, low_hz=low_hz, high_hz=high_hz, prefer_gpu=prefer_gpu, require_gpu=require_gpu)
        r["node_id"] = "temporal_filtering_subject"; return r
