...
def _resolve_tr_from_bids_sidecar(
    input_path: Path, subject_id: str, derivatives_dir: str
...
def _resolve_tr(
    ...
    if tr is not None:
        try:
            ...
            return None, warnings, errors, "parameter"
    sidecar_tr, sidecar_warnings, sidecar_errors, sidecar_source = _resolve_tr_from_bids_sidecar(
        input_path, subject_id, derivatives_dir
    )
...
def _write_qc_markdown(path: Path, qc: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        f"- OK: {qc.get('ok')}",
        f"- Status: {qc.get('filtering_qc_status')}",
        f"- Input: `{qc.get('input_nii')}`",
        f"- Output: `{qc.get('output_nii')}`",
        f"- TR: {qc.get('tr')}",
        f"- Band: {qc.get('low_hz')} - {qc.get('high_hz')} Hz",
        f"- Nyquist: {qc.get('nyquist_hz')} Hz",
        f"- Retained frequency bins: {qc.get('retained_frequency_bin_count')} / {qc.get('frequency_bin_count')}",
...
def run_python_temporal_filter_subject(
    ...
    try:
        ...
        return _failure(
            ...
            qc_json,
            qc_md,
            ["low_hz and high_hz must be numeric."],
            warnings,
        )
    ...
    try:
        ...
        if high_hz >= nyquist:
            warnings.append(f"high_hz={high_hz} >= Nyquist={nyquist}. Clipping high_hz to Nyquist.")
            high_hz = nyquist
        ...
        finite_fraction = (
            float(np.count_nonzero(finite_mask) / filtered.size) if filtered.size else 0.0
        )
...
def write_temporal_filtering_dataset_report(
    ...
    rf = [
        float(s["retained_frequency_fraction"])
        for s in subjects
        if s.get("retained_frequency_fraction") is not None
    ]
    summary = {
        "ok": n > 0 and fail_count == 0,
        ...
        "mean_variance_ratio": float(mean(vr)) if vr else None,
        "mean_retained_frequency_fraction": float(mean(rf)) if rf else None,
        "subjects": subjects,
        "warnings": warnings,
        "errors": errors,
    }
    sp = report_out / "temporal_filtering_qc_summary.json"
    rp = report_out / "temporal_filtering_qc_report.md"
    atomic_write_json(sp, summary, schema_version=1)
    lines = [
        "# rs-fMRI Temporal Filtering QC Dataset Report",
        "",
        "## Summary",
        "",
        f"- Subjects total: {n}",
        ...
        f"- FAIL: {fail_count}",
        f"- Mean variance ratio: {summary['mean_variance_ratio']}",
        f"- Mean retained frequency fraction: {summary['mean_retained_frequency_fraction']}",
        "",
        "## Subjects",
        "",
        "| Subject | Status | TR | Band Hz | Retained Bins | Variance Ratio |",
        "|---|---|---:|---|---:|---:|",
    ]
    for s in subjects:
        lines.append(
            f"| {s.get('subject_id')} | {s.get('filtering_qc_status')} | {s.get('tr')} | {s.get('low_hz')}-{s.get('high_hz')} | {s.get('retained_frequency_bin_count')} | {s.get('variance_ratio')} |"
        )
    lines += [
        "",
        "## Safety Note",
        "",
        "This report summarizes derivative temporal filtering QC outputs only. It does not modify rawdata.",
    ]
    rp.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return {
        "ok": True,
