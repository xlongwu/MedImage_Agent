...
from src.backend.app.tools.gpu_nuisance_regression_runner import (
    run_nuisance_regression_subject as run_gpu_nuisance_regression,
)
...
def _find_realigned_functional(subject_id: str, derivatives_dir: str) -> Path | None:
    ...
    candidates = sorted(
        path
        for path in func_dir.glob("r*.nii*")
        if path.is_file() and ("bold" in path.name.lower() or "rest" in path.name.lower())
    )
    return candidates[0] if candidates else None


def _find_functional_for_nuisance(
    subject_id: str, derivatives_dir: str, input_nii: str | None = None
) -> Path | None:
    if input_nii:
        return Path(input_nii)
    return _find_smoothed_functional(subject_id, derivatives_dir) or _find_realigned_functional(
        subject_id, derivatives_dir
    )
...
def run_nuisance_regression_subject(
    ...
    if backend == "gpu":
        ...
        if not input_func:
            return {
                ...
                "warnings": [],
                "errors": [
                    f"No realigned or smoothed functional input found for subject {subject_id}."
                ],
            }
        motion = _find_motion_params(
            subject_id, derivatives_dir, motion_parameter_file=motion_parameter_file
        )
        if not motion:
            return {
                "ok": False,
        ...
        r["node_id"] = "nuisance_regression_subject"
        return r
    ...
    if not _is_safe_subject_func_path(
        input_func, subject_id, derivatives_dir
    ) or not input_func.name.startswith(("swr", "swra", "r", "ra", "realigned_")):
        return {
            "ok": False,
            "node_id": "nuisance_regression_subject",
            "backend": "python",
            ...
            "outputs": [],
            "warnings": [],
            "errors": [f"Unsafe nuisance functional input: {input_func}"],
        }
