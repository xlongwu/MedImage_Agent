...
def main() -> int:
    ...
    project_config = (
        Path(args[0]) if len(args) > 0 else Path("examples/project_config_dataset.yaml")
    )
    pipeline = (
        Path(args[1]) if len(args) > 1 else Path("examples/pipeline_rsfmri_smoothing_qc.yaml")
    )
    if approved:
        pipeline = _make_approved_pipeline_copy(
            source=pipeline, target=Path("outputs/work/rsfmri/approved_pipeline_smoothing_qc.yaml")
        )
