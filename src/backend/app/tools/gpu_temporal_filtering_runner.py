...
def run_gpu_temporal_filtering_subject(
    *,
    ...
    result: dict = {
        "ok": True,
        "node_id": "gpu_temporal_filtering_subject",
        "backend": "gpu",
    ...
    if not approved:
        result["ok"] = False
        result["errors"].append("GPU temporal filtering requires approved=true.")
        return result
    ...
    if not is_scoped_derivative_path(input_path, derivatives):
        result["ok"] = False
        result["errors"].append(f"Input not under derivatives_dir: {input_functional}")
        return result

    # TR
    if (
        not isinstance(tr, int | float)
        or tr != tr
        or tr == float("inf")
        or tr <= 0
        or tr < 0.1
    ...
    if (
        ...
        result["ok"] = False
        result["errors"].append("Frequency band values must be finite numeric.")
        return result
    if low <= 0 or high <= low or high >= nyquist:
        result["ok"] = False
        result["errors"].append(
            f"Invalid band [{low},{high}] TR={tr} Nyq={nyquist:.3f}. Need 0<low<high<Nyquist."
        )
        return result
    ...
    if filter_mode not in _ALLOWED_MODES:
        result["ok"] = False
        result["errors"].append(f"Invalid filter_mode: {filter_mode}. Allowed: bandpass.")
        return result
    if filter_method not in _ALLOWED_METHODS:
        result["ok"] = False
        result["errors"].append(f"Invalid filter_method: {filter_method}. Allowed: butterworth.")
        return result
    if not isinstance(filter_order, int) or filter_order < 1 or filter_order > 4:
        result["ok"] = False
        result["errors"].append(f"Invalid filter_order: {filter_order}. Must be 1–4.")
        return result
