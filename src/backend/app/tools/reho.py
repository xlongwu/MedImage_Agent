...
def _find_filtered(subject_id: str, derivatives_dir: str) -> Path | None:
    ...
    if p.exists():
        return p
    c = sorted(d.glob("filt_resid_swr*.nii"))
...
def _fail(
    sid: str, rj: Path, qj: Path, qm: Path, errors: list[str], warnings: list[str] | None = None
) -> dict[str, Any]:
    w = warnings or []
    qc = {
        "ok": False,
...
def run_python_reho_subject(
    subject_id: str, derivatives_dir: str, neighborhood: int = 27, use_gm_mask: bool = False
) -> dict[str, Any]:
    try:
    ...
    if not ip:
        return _fail(subject_id, rj, qj, qm, ["No filtered functional input found."])
    if not _safe_filtered(ip, subject_id, derivatives_dir):
        return _fail(subject_id, rj, qj, qm, [f"Unsafe input: {ip}"])
    ...
    try:
        ...
        if data.ndim != 4:
            raise ValueError(f"Must be 4D. Got {data.shape}")
        nx, ny, nz, nt = data.shape
        if nt < 2:
            raise ValueError(f"Need >=2 timepoints, got {nt}.")
        ...
        if use_gm_mask:
            ...
            if gp:
                ...
                if list(gd.shape[:3]) == [nx, ny, nz]:
                    ...
                    w.append("GM shape mismatch; ignoring mask.")
            else:
                w.append("GM map not found; computing on internal voxels.")
        ...
        h3 = img.header.copy()
        try:
            h3.set_data_shape(reho.shape)
        except Exception:
            pass
        nib.save(nib.Nifti1Image(reho, affine=img.affine, header=h3), str(rf))

        fm = np.isfinite(reho)
        ff = float(np.count_nonzero(fm) / reho.size) if reho.size else 0.0
        nz = reho[reho != 0]
        if nz.size > 0:
        ...
        if vc == 0:
            status = "FAIL"
            e.append("No valid voxels.")
        elif ff < 0.95:
            status = "WARNING"
            w.append("Finite fraction below 0.95.")
        elif rmin < -1e-6 or rmax > 1.000001:
            status = "WARNING"
            w.append(f"ReHo out of [0,1]: min={rmin}, max={rmax}")
        ...
        qc = {
            ...
            "output_shape": list(reho.shape),
            "timepoints": int(nt),
            "neighborhood": nb,
            "neighbor_count": len(reho_compute._offsets(nb)),
            "boundary_strategy": "skip_boundary",
        ...
        result = {
            ...
            "reho_file": str(rf),
            "qc": qc,
            "outputs": [str(rf), str(rj), str(qj), str(qm)],
            "warnings": w,
            "errors": e,
        }
...
def write_reho_dataset_report(derivatives_dir: str, report_dir: str) -> dict[str, Any]:
    ...
    summary = {
        ...
        "subjects_warning": wc,
        "subjects_fail": fc,
        "mean_reho_mean": float(mean(rm)) if rm else None,
        "mean_valid_voxel_count": float(mean(vc)) if vc else None,
        "subjects": subjects,
        "warnings": w,
        "errors": e,
    }
    sp = ro / "reho_qc_summary.json"
    rp = ro / "reho_qc_report.md"
    sp.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# rs-fMRI ReHo QC Dataset Report",
        "",
        "## Summary",
        "",
        f"- Subjects: {n}",
        ...
        f"- WARNING: {wc}",
        f"- FAIL: {fc}",
        f"- Mean ReHo mean: {summary['mean_reho_mean']}",
        f"- Mean valid voxels: {summary['mean_valid_voxel_count']}",
        "",
        "## Subjects",
        "",
        "| Subject | Status | Nb | Valid Voxels | ReHo Mean | ReHo Max |",
        "|---|---|---:|---:|---:|---:|",
    ]
    for s in subjects:
        lines.append(
            f"| {s.get('subject_id')} | {s.get('reho_qc_status')} | {s.get('neighborhood')} | {s.get('valid_voxel_count')} | {s.get('reho_mean')} | {s.get('reho_max')} |"
        )
    lines += [
        "",
        "## Safety Note",
        "",
        "This report summarizes derivative ReHo QC outputs only. It does not modify rawdata.",
    ]
    rp.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return {
        "ok": True,
