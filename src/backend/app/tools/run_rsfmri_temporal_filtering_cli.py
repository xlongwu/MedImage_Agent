...
def main() -> int:
    ...
    pl = Path(args[1]) if len(args) > 1 else Path("examples/pipeline_rsfmri_temporal_filtering.yaml")
    if approved: pl = _make_approved_pipeline_copy(pl, Path("outputs/work/rsfmri/approved_pipeline_temporal_filtering.yaml"))
