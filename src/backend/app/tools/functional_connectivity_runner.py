...
from src.backend.app.tools.gpu_functional_connectivity_runner import run_functional_connectivity_subject as run_gpu_fc

def run_functional_connectivity_subject(subject_id: str, derivatives_dir: str, backend: str = "python", roi_count: int = 4, atlas_path: str | None = None, generate_seed_map: bool = False, input_nii: str | None = None, prefer_gpu: bool = True, require_gpu: bool = False) -> dict[str, Any]:
    if backend == "gpu":
        if not input_nii:
            return {"ok": False, "node_id": "functional_connectivity_subject", "backend": backend, "subject_id": subject_id, "outputs": [], "warnings": [], "errors": ["GPU backend requires input_nii."]}
        r = run_gpu_fc(subject_id=subject_id, input_nii=input_nii, derivatives_dir=derivatives_dir, roi_count=roi_count, atlas_path=atlas_path, generate_seed_map=generate_seed_map, prefer_gpu=prefer_gpu, require_gpu=require_gpu)
