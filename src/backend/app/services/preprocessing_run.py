...
def _discover_input_inventory(input_dir: Path) -> dict[str, Any]:
    ...
    sessions: set[str] = set(); bids_entities: list[dict[str, Any]] = []
    ...
    return {"subjects": all_subjects, "bold_files": bold_files, "t1w_files": t1w_files,
            ...
            "sessions": sorted(sessions), "bids_entities": bids_entities,
...
def create_preprocessing_run(
    ...
    *,
    ...
    source_registry_path = request.input_registry_path or str(meta.get("preprocessing_input_registry_path") or "")
    conversion_run_id = request.conversion_run_id or str(meta.get("preprocessing_conversion_run_id") or "")
    source_kind = request.source_kind or str(meta.get("preprocessing_input_source") or "external_import")
    ...
    if not registry.ok:
        return PreprocessingRunCreateResponse(ok=False, status="blocked", project_id=project_id,
            preprocessing_run_id=run_id, run_dir=str(run_dir),
            preprocessing_input_dir=input_dir,
            blocking_issues=registry.blocking_issues,
            warnings=registry.warnings, errors=registry.errors,
    ...
    manifest = {"project_id": project_id, "preprocessing_run_id": run_id,
                "status": "created", "input_dir": input_dir,
                "source_kind": source_kind, "conversion_run_id": conversion_run_id,
...
def execute_python_preflight(
    ...
    *,
    ...
    manifest = {"project_id": project_id, "preprocessing_run_id": preprocessing_run_id,
                ...
                "stage_statuses": [s.model_dump() for s in stages],
                "artifacts": {"input_inventory": str(inv_path),
    ...
    *,
    ...
    if manifest_path.exists():
        ...
        return PreprocessingRunStatusResponse(
            ...
            artifact_registry_path=str(data.get("artifact_registry_path") or data.get("artifacts", {}).get("artifact_registry", "")),
    ...
    for stage in all_stages:
        ...
        try:
            ...
            if result["ok"]:
                ...
                output_status = str(output.get("status", "succeeded")) if isinstance(output, dict) else "succeeded"
                stage.status = normalize_stage_execution_status(
                    output_status,
                    metadata_only=bool(isinstance(output, dict) and output.get("metadata_only", False)),
                    preview_only=bool(isinstance(output, dict) and output.get("preview_only", False)),
                )
                ...
                if stage.status == "succeeded":
                    ...
                    warnings.append(f"{stage.stage_id}: metadata_only output was not promoted to succeeded.")
                elif stage.status == "preview_only":
                    preview_only_list.append(stage.stage_id)
                    warnings.append(f"{stage.stage_id}: preview_only output was not promoted to succeeded.")
