...
from src.backend.app.schemas.preprocessing_spm_execution import (
    SpmSandboxExecutionRequest,
    SpmSandboxExecutionResponse,
    sandbox_safety_flags,
...
def run_sandbox_spm_execution(
    project_id: str,
    run_id: str,
    request: SpmSandboxExecutionRequest,
    *,
    project_dir: str = "",
    ...
    exec_id = (
        "spm-ex-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
    )
    exec_dir = (
    ...
    bold_files = [
        ...
        if ("bold" in p.name.lower() or "rest" in p.name.lower()) and p.is_file()
    ]
    if not bold_files:
        return SpmSandboxExecutionResponse(
            ok=False,
            status="blocked",
            project_id=project_id,
            blocking_issues=["No BOLD files found."],
            safety_flags=sandbox_safety_flags(),
        )
    ...
    for i, cp in enumerate(copied):
        ...
        batch_lines.append(
            f"matlabbatch{{{i * 2 + 2}}}.spm.spatial.realign.estwrite.data={{'{cp},1'}};"
        )
