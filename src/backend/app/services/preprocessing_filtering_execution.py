...
def run_filtering_sandbox_execution(
    project_id: str,
    run_id: str,
    request: FilteringSandboxExecutionRequest,
    *,
    project_dir: str = "",
    ...
    exec_id = (
        "tf-ex-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
    )
    exec_dir = (
    ...
    if _SCIPY_OK:
        for cp, design in zip(copied, designs, strict=False):
            try:
                ...
                filt_img = _nib.Nifti1Image(
                    filtered_data.astype(_np.float32), img.affine, img.header
                )
                out_path = sandbox_out / design["subject"] / f"filtered_{cp.name}"
                out_path.parent.mkdir(parents=True, exist_ok=True)
                _nib.save(filt_img, str(out_path))
                design["filtered"] = True
    ...
    (exec_dir / "subject_status.json").write_text(
        json.dumps({"total": len(copied), "filtered": filtered, "metadata_only": metadata_only})
    )
    ...
    return FilteringSandboxExecutionResponse(
        ...
        subjects_succeeded=filtered,
        subjects_failed=len(copied) - filtered,
        filter_design_path=str(fd_path),
        ...
        warnings=warnings,
        next_actions=["Review filter results.", "Plan ALFF/ReHo dry-run."],
        safety_flags=filtering_safety_flags(),
    )
