...
from src.backend.app.schemas.spm_runtime import (
    SpmRuntimePreflightResponse, SpmSyntheticSmokeRequest, SpmSyntheticSmokeResponse,
    _REQUIRED_SYNTHETIC_SPM_FLAGS, validate_synthetic_spm_env, safety_flags_spm,
)
...
def spm_runtime_preflight(project_id: str, *, env: dict[str, str] | None = None) -> SpmRuntimePreflightResponse:
    warnings: list[str] = []; errors: list[str] = []
    eff_env = env or dict(os.environ)
    # Check env flags
    ok_flags, missing = validate_synthetic_spm_env(eff_env)
    if not ok_flags:
        return SpmRuntimePreflightResponse(ok=True, status="disabled", project_id=project_id,
    ...
    if not matlab_path:
        return SpmRuntimePreflightResponse(ok=False, status="blocked", project_id=project_id,
    ...
    if not spm_found:
        return SpmRuntimePreflightResponse(ok=False, status="blocked", project_id=project_id,
            matlab_available=True, matlab_executable=matlab_path,
            spm_path=spm_path, errors=[f"SPM directory not found: {spm_path}"],
            safety_flags=safety_flags_spm())

    return SpmRuntimePreflightResponse(ok=True, status="ready_for_synthetic_smoke", project_id=project_id,
        matlab_available=True, matlab_executable=matlab_path,
        spm_available=True, spm_path=spm_path,
        required_env_flags=list(_REQUIRED_SYNTHETIC_SPM_FLAGS),
...
def run_synthetic_spm_smoke(
    project_id: str, request: SpmSyntheticSmokeRequest,
    *, project_dir: str = "", env: dict[str, str] | None = None
) -> SpmSyntheticSmokeResponse:
    eff_env = env or dict(os.environ)
    ok_flags, missing = validate_synthetic_spm_env(eff_env)
    if not ok_flags:
        return SpmSyntheticSmokeResponse(ok=False, status="disabled", project_id=project_id,
            blocking_issues=[f"Missing env flags: {missing}"], safety_flags=safety_flags_spm())
    ...
    input_dir = smoke_dir / "input"; input_dir.mkdir()
    output_dir = smoke_dir / "output"; output_dir.mkdir()
    logs_dir = smoke_dir / "logs"; logs_dir.mkdir()

    # Generate synthetic NIfTI
    try:
        nifti_path = _gen_synthetic_nifti(input_dir)
    except ImportError:
        return SpmSyntheticSmokeResponse(ok=False, status="blocked", project_id=project_id,
            smoke_run_id=smoke_id, blocking_issues=["nibabel not available for synthetic NIfTI generation."],
            safety_flags=safety_flags_spm())
...
"""
    ...
    template_path.write_text(json.dumps({"tool": "matlab", "executable": request.matlab_executable,
        "args": ["-nodisplay", "-nosplash", "-nodesktop", "-r", f"run('{batch_path}');exit;"],
        "shell": False, "synthetic_only": True}, indent=2), encoding="utf-8")

    # Write manifest + provenance stubs
    (smoke_dir / "manifest.json").write_text(json.dumps({"smoke_id": smoke_id, "status": "generated"}, indent=2))
    (smoke_dir / "provenance.json").write_text(json.dumps({"smoke_id": smoke_id, "synthetic_only": True}, indent=2))
    (smoke_dir / "README.md").write_text(f"# SPM Synthetic Smoke {smoke_id}\nResearch use only.\n")

    return SpmSyntheticSmokeResponse(
        ok=True, status="generated", project_id=project_id, smoke_run_id=smoke_id,
        smoke_dir=str(smoke_dir), synthetic_input_path=str(nifti_path),
        synthetic_output_dir=str(output_dir), command_template_path=str(template_path),
        batch_script_path=str(batch_path),
        stdout_log_path=str(logs_dir / "stdout.log"), stderr_log_path=str(logs_dir / "stderr.log"),
        manifest_path=str(smoke_dir / "manifest.json"), provenance_path=str(smoke_dir / "provenance.json"),
