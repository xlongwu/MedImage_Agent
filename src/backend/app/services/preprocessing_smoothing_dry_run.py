...
from src.backend.app.schemas.preprocessing_smoothing_dry_run import (
    SmoothingDryRunRequest, SmoothingDryRunResponse, smoothing_safety_flags,
)
...
def run_smoothing_dry_run(
    project_id: str, run_id: str, request: SmoothingDryRunRequest,
    *, project_dir: str = ""
    ...
    if not func_input:
        return SmoothingDryRunResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=["No functional input registered."], safety_flags=smoothing_safety_flags())
    ...
    norm_bolds = [p for p in sorted(func_path.rglob("*.nii*")) if p.is_file() and ("bold" in p.name.lower() or "rest" in p.name.lower())]
    if not norm_bolds:
        return SmoothingDryRunResponse(ok=False, status="blocked", project_id=project_id,
    ...
    # Create dry-run dir
    dry_id = "s-dr-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
...
"""
    ...
    (dry_dir / "smoothing_dry_run_manifest.json").write_text(json.dumps({
        "dry_run_id": dry_id, "status": "dry_run_preview", "subjects": len(subjects)}, indent=2))
    (dry_dir / "subject_planning_summary.json").write_text(json.dumps({"total": len(subjects)}, indent=2))
    ...
    return SmoothingDryRunResponse(
        ...
        warnings=warnings, next_actions=["Review batch preview.", "Enable MATLAB/SPM for execution."],
        safety_flags=smoothing_safety_flags())
