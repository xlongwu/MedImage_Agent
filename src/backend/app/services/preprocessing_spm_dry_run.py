...
from src.backend.app.schemas.preprocessing_spm_dry_run import (
    SliceTimingRealignDryRunRequest, SliceTimingRealignDryRunResponse, safety_flags_dry_run,
)
...
def run_slice_timing_realign_dry_run(
    project_id: str, run_id: str, request: SliceTimingRealignDryRunRequest,
    *, project_dir: str = ""
) -> SliceTimingRealignDryRunResponse:
    blocking: list[str] = []; warnings: list[str] = []; errors: list[str] = []

    project = mock_store.get_project(project_id)
    if not project:
        return SliceTimingRealignDryRunResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=[f"Project not found: {project_id}"], safety_flags=safety_flags_dry_run())
    ...
    if not input_dir:
        return SliceTimingRealignDryRunResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=["No preprocessing input registered."], safety_flags=safety_flags_dry_run())

    rawdata_dir = str(meta.get("rawdata_dir") or "")
    input_path = Path(input_dir)
    if not input_path.exists():
        return SliceTimingRealignDryRunResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=[f"Input dir not found: {input_dir}"], safety_flags=safety_flags_dry_run())
    ...
    if not bold_files:
        return SliceTimingRealignDryRunResponse(ok=False, status="blocked", project_id=project_id,
            preprocessing_run_id=run_id, blocking_issues=["No BOLD files found in preprocessing input."],
    ...
    if blocking:
        return SliceTimingRealignDryRunResponse(ok=False, status="blocked", project_id=project_id,
            preprocessing_run_id=run_id, blocking_issues=blocking,
            safety_flags=safety_flags_dry_run())

    # Create dry-run directory
    effective_pd = project_dir or str(meta.get("project_dir") or "")
    dry_id = "spm-dr-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
...
"""
    ...
    # Write manifest
    manifest = {"project_id": project_id, "run_id": run_id, "dry_run_id": dry_id,
                "status": "dry_run_preview", "subject_count": len(subjects),
                "planned_subjects": planned, "blocked_subjects": blocked_subjects,
                "bold_count": len(bold_files), "created_at": _now_iso()}
    (dry_dir / "dry_run_manifest.json").write_text(json.dumps(manifest, indent=2))
    (dry_dir / "subject_planning_summary.json").write_text(json.dumps(
        {"planned": planned, "blocked": blocked_subjects, "params": {"tr": tr, "nslices": nslices, "ref_slice": ref_slice}}, indent=2))
    (dry_dir / "README.md").write_text(f"# SPM Dry-Run {dry_id}\nDry-run only. MATLAB/SPM not executed. No image outputs.\n")
    ...
    return SliceTimingRealignDryRunResponse(
        ...
        dry_run_id=dry_id, dry_run_dir=str(dry_dir),
        subject_count=len(subjects), planned_subjects=planned,
        blocked_subjects=blocked_subjects, bold_count=len(bold_files),
