...
from src.backend.app.schemas.preprocessing_filtering_dry_run import (
    FilteringDryRunRequest, FilteringDryRunResponse, filtering_safety_flags,
)
...
def run_filtering_dry_run(
    project_id: str, run_id: str, request: FilteringDryRunRequest,
    *, project_dir: str = ""
    ...
    if not func_input:
        return FilteringDryRunResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=["No functional input registered."], safety_flags=filtering_safety_flags())

    func_path = Path(func_input)
    if not func_path.exists():
        return FilteringDryRunResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=[f"Functional input not found: {func_input}"], safety_flags=filtering_safety_flags())
    ...
    dry_id = "tf-dr-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
    ...
    subjects: set[str] = set(); filter_paths: list[str] = []; output_paths: list[str] = []
    for bf in bold_files[:10]:
        ...
        design = {"subject": subj, "functional": str(bf), "low_cut_hz": low, "high_cut_hz": high,
                  "method": "bandpass", "planned_output": f"filtered_{bf.name}"}
    ...
    (dry_dir / "filtering_dry_run_manifest.json").write_text(json.dumps({
    ...
    return FilteringDryRunResponse(
        ...
        filter_design_paths=filter_paths, planned_output_paths=output_paths,
        warnings=warnings, next_actions=["Review filter designs.", "Enable execution for temporal filtering."],
