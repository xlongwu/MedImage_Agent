...
def register_converted_bids_as_preprocessing_input(
    ...
    *,
    ...
    if not converted_bids_dir:
        converted_bids_dir = (
            f"{effective_project_dir}/conversion_runs/{request.conversion_run_id}"
    ...
    registry_result = write_converted_input_registry(
        ...
        manifest_path=request.manifest_path or str(manifest_path)
        if manifest_found
    ...
    missing_sidecars = [
        item for item in inventory.get("missing_sidecar_pairings", []) if isinstance(item, dict)
    ]
    ...
    return PreprocessingInputRegistrationResponse(
        ...
        bids_entities=[
            item for item in inventory.get("bids_entities", []) if isinstance(item, dict)
        ],
