...
def _append_registered_artifacts(
    *,
    ...
    filtered = {artifact_type: paths for artifact_type, paths in output_paths_by_type.items() if paths}
    if not filtered:
        return
    run_dir = _run_dir(effective_project_dir, run_id)
    exec_dir = run_dir / "spm_exec" / execution_id
    provenance_path = str(exec_dir / "provenance.json") if (exec_dir / "provenance.json").exists() else ""
...
def _compute_motion_qc_artifacts(
    *,
    ...
    if not motion_files:
        return [], [], [], [], ["No motion parameter files found; motion QC could not be generated."]
    run_dir = _run_dir(effective_project_dir, run_id)
    derivatives_dir = Path(effective_project_dir) / "derivatives" if effective_project_dir else run_dir / "derivatives"
...
def register_sandbox_spm_outputs(
    ...
    *, project_dir: str = ""
    ...
    _write_json(reg_dir / "stage_output_registry.json", {
        "stage_output_id": stage_out_id, "source_execution": request.execution_id,
        "stage": "slice_timing_realign", "status": "registered", "created_at": _now_iso()})
    motion_qc_jsons, fd_tsvs, motion_qc_summaries, motion_qc_markdowns, motion_qc_warnings = _compute_motion_qc_artifacts(
    ...
    _write_json(reg_dir / "next_stage_input_manifest.json", {
    ...
    _write_json(reg_dir / "subject_output_summary.json", {
    ...
    return StageOutputRegistrationResponse(
        ...
        motion_files=[str(f) for f in motion_files], mean_images=[str(f) for f in mean_images],
        warnings=warnings,
        next_actions=["Review registered outputs and motion QC.", "Plan nuisance regression with motion parameters."],
        safety_flags=registration_safety_flags())
...
def register_coreg_norm_outputs(
    ...
    *, project_dir: str = ""
    ...
    norm_bolds = [
        p for p in nifti_outputs
        if ("bold" in p.name.lower() or "rest" in p.name.lower())
        and p.name.lower().startswith("w")
    ]
    t1w_outputs = [
        p for p in nifti_outputs
        if "t1" in p.name.lower()
        and not p.name.lower().startswith(("c1", "c2", "c3", "y_"))
    ]
    seg_maps = [
        p for p in nifti_outputs
    ...
    _write_json(reg_dir / "coreg_norm_stage_output_registry.json", {
        "stage_output_id": stage_out_id, "source_execution": request.execution_id,
        "stage": "coreg_norm", "status": "registered"})
    _write_json(reg_dir / "next_stage_input_manifest.json", {
        "next_stage_input_dir": str(sandbox_out), "func_count": len(norm_bolds),
        "t1w_count": len(t1w_outputs), "segmentation_maps": len(seg_maps)})
    ...
    return StageOutputRegistrationResponse(
        ...
        motion_files=[str(p) for p in t1w_outputs], mean_images=[str(p) for p in seg_maps],
...
def register_smoothing_outputs(
    ...
    *, project_dir: str = ""
    ...
    _write_json(reg_dir / "smoothing_stage_output_registry.json", {
        "stage_output_id": stage_out_id, "source_execution": request.execution_id, "stage": "smoothing"})
    _write_json(reg_dir / "next_stage_input_manifest.json", {
...
def register_nuisance_outputs(
    ...
    *, project_dir: str = ""
    ...
    if metadata_only:
        ...
        _write_json(reg_dir / "nuisance_stage_output_registry.json", {
            "stage_output_id": stage_out_id, "status": "not_ready_for_filtering", "metadata_only": True})
    else:
        _write_json(reg_dir / "nuisance_stage_output_registry.json", {
...
def register_filtering_outputs(
    ...
    *, project_dir: str = ""
    ...
    _write_json(reg_dir / "filtering_stage_output_registry.json", {
        "stage_output_id": stage_out_id, "status": "registered" if not metadata_only else "not_ready_for_alff_reho"})
...
def register_alff_reho_outputs(
    ...
    *, project_dir: str = ""
    ...
    alff_provenance = [
        p for p in sorted(sandbox_out.rglob("*alff*provenance.json"))
        if p.is_file()
    ]
    reho_provenance = [
        p for p in sorted(sandbox_out.rglob("*reho*provenance.json"))
        if p.is_file()
    ]
    stage_manifests = [
        p for p in [
    ...
    _write_json(reg_dir / "alff_reho_stage_output_registry.json", {
        "stage_output_id": stage_out_id, "metric_outputs_ready": metric_ready,
        "alff_count": len(alff_files), "falff_count": len(falff_files),
...
def register_fc_outputs(
    ...
    *, project_dir: str = ""
    ...
    fc_files = [
        p for p in sorted(sandbox_out.rglob("*"))
        if p.is_file()
        and (
    ...
    _write_json(reg_dir / "fc_stage_output_registry.json", {
        "stage_output_id": stage_out_id, "fc_outputs_ready": ready,
        "fc_matrix_count": len(fc_files), "fisher_z_count": len(fz_files)})
