...
def run_fc_dry_run(
    project_id: str, run_id: str, request: FcDryRunRequest, *, project_dir: str = ""
) -> FcDryRunResponse:
    blocking: list[str] = []
    warnings: list[str] = []
    ...
    dry_id = (
        "fc-dr-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
    )
    dry_dir = (
    ...
    for bf in bold_files[:10]:
        ...
        plan = {
            ...
            "fisher_z": request.fisher_z,
            "atlas": request.atlas_name or "none",
    ...
    (dry_dir / "fc_dry_run_manifest.json").write_text(
        json.dumps(
    ...
    return FcDryRunResponse(
        ...
        atlas_status=atlas_status,
        fc_plan_paths=fc_paths,
        planned_output_paths=output_paths,
        warnings=warnings,
        next_actions=["Review FC plan.", "Enable FC execution."],
        safety_flags=fc_safety_flags(),
    )
