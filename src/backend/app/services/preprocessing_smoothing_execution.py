...
from src.backend.app.schemas.preprocessing_smoothing_execution import (
    SmoothingSandboxExecutionRequest, SmoothingSandboxExecutionResponse,
    validate_smoothing_env, smoothing_safety_flags,
)
...
def run_smoothing_sandbox_execution(
    project_id: str, run_id: str, request: SmoothingSandboxExecutionRequest,
    *, project_dir: str = "", env: dict[str, str] | None = None
    ...
    if not ok_flags:
        return SmoothingSandboxExecutionResponse(ok=False, status="disabled", project_id=project_id,
            blocking_issues=[f"Missing env flags: {missing}"], safety_flags=smoothing_safety_flags())
    ...
    if not dry_dir or not dry_dir.exists():
        return SmoothingSandboxExecutionResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=[f"Dry-run not found: {request.dry_run_id}"], safety_flags=smoothing_safety_flags())
    ...
    if not func_path or not func_path.exists():
        return SmoothingSandboxExecutionResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=["Functional input not found."], safety_flags=smoothing_safety_flags())

    bold_files = [p for p in sorted(func_path.rglob("*.nii*")) if p.is_file() and ("bold" in p.name.lower() or "rest" in p.name.lower())]

    exec_id = "s-ex-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
    ...
    (exec_dir / "manifest.json").write_text(json.dumps({"status": "succeeded" if rc == 0 else "failed"}))
    ...
    return SmoothingSandboxExecutionResponse(
        ...
        batch_script_path=str(batch_path), stdout_log_path=str(stdout_log),
