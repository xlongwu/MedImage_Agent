...
def run_alff_reho_dry_run(
    project_id: str, run_id: str, request: AlffRehoDryRunRequest, *, project_dir: str = ""
) -> AlffRehoDryRunResponse:
    blocking: list[str] = []
    warnings: list[str] = []
    ...
    dry_id = (
        "ar-dr-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
    )
    dry_dir = (
    ...
    for bf in bold_files[:10]:
        ...
        plan = {
            "subject": subj,
            "functional": str(bf),
            "alff": request.compute_alff,
            "falff": request.compute_falff,
            "reho": request.compute_reho,
    ...
    (dry_dir / "alff_reho_dry_run_manifest.json").write_text(
        json.dumps(
    ...
    return AlffRehoDryRunResponse(
        ...
        alff_plan_paths=alff_paths,
        reho_plan_paths=reho_paths,
