...
from src.backend.app.schemas.preprocessing_coreg_norm_execution import (
    CoregNormSandboxExecutionRequest, CoregNormSandboxExecutionResponse,
    validate_coreg_norm_env, coreg_norm_safety_flags,
)
...
def run_coreg_norm_sandbox_execution(
    project_id: str, run_id: str, request: CoregNormSandboxExecutionRequest,
    *, project_dir: str = "", env: dict[str, str] | None = None
    ...
    if not ok_flags:
        return CoregNormSandboxExecutionResponse(ok=False, status="disabled", project_id=project_id,
            blocking_issues=[f"Missing env flags: {missing}"], safety_flags=coreg_norm_safety_flags())
    ...
    if not dry_dir or not dry_dir.exists():
        return CoregNormSandboxExecutionResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=[f"Dry-run not found: {request.dry_run_id}"], safety_flags=coreg_norm_safety_flags())
    ...
    if not func_path or not func_path.exists():
        return CoregNormSandboxExecutionResponse(ok=False, status="blocked", project_id=project_id,
            blocking_issues=["Functional input not found."], safety_flags=coreg_norm_safety_flags())
    ...
    t1w_files = [p for p in sorted(t1w_path.rglob("*.nii*")) if p.is_file() and "t1" in p.name.lower()] if t1w_path else []

    # Create sandbox execution directory
    exec_id = "cn-ex-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
    ...
    for tf in t1w_files[:10]:
        ...
        for part in tf.parts:
            if part.startswith("sub-"): subj = part; break
        dest = sandbox_in / subj / "anat"; dest.mkdir(parents=True, exist_ok=True)
        shutil.copy2(tf, dest / tf.name)
        copied_t1w.append(dest / tf.name)

    # Generate batch script
    batch_lines = ["%% SPM Coregistration + Normalization (Sandbox)", "spm('defaults','FMRI');", "matlabbatch={};"]
    bid = 0
    for i, cf in enumerate(copied_func):
        ...
        bid += 1; batch_lines.append(f"matlabbatch{{{bid}}}.spm.spatial.coreg.estimate.ref={{'{cf},1'}};")
        batch_lines.append(f"matlabbatch{{{bid}}}.spm.spatial.coreg.estimate.source={t1_ref};")
        bid += 1; batch_lines.append(f"matlabbatch{{{bid}}}.spm.spatial.preproc.channel.vols={t1_ref};")
        bid += 1; batch_lines.append(f"matlabbatch{{{bid}}}.spm.spatial.normalise.estwrite.subj.vol={{'{cf},1'}};")
    ...
    # Manifest/provenance
    (exec_dir / "manifest.json").write_text(json.dumps({"exec_id": exec_id, "status": "succeeded" if rc == 0 else "failed"}))
    (exec_dir / "provenance.json").write_text(json.dumps({"exec_id": exec_id, "sandbox_only": True}))
    (exec_dir / "subject_status.json").write_text(json.dumps({"total": len(copied_func), "succeeded": len(copied_func) if rc == 0 else 0}))
    ...
    return CoregNormSandboxExecutionResponse(
        ...
        subjects_total=len(copied_func), subjects_succeeded=len(copied_func) if rc == 0 else 0,
