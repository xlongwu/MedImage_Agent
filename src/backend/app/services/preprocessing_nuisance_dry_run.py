...
def run_nuisance_dry_run(
    project_id: str, run_id: str, request: NuisanceDryRunRequest, *, project_dir: str = ""
) -> NuisanceDryRunResponse:
    warnings: list[str] = []
    _blocking: list[str] = []
    ...
    if not smooth_bolds:
        return NuisanceDryRunResponse(
            ...
            status="blocked",
            project_id=project_id,
            blocking_issues=["No smoothed functional files found."],
            safety_flags=nuisance_safety_flags(),
        )
    ...
    dry_id = (
        "nr-dr-" + hashlib.sha256(f"{project_id}:{run_id}:{_now_iso()}".encode()).hexdigest()[:10]
    )
    dry_dir = (
    ...
    for bf in smooth_bolds[:10]:
        ...
        regressors = [
            "motion_24" if request.include_motion_24 else "",
    ...
    (dry_dir / "nuisance_dry_run_manifest.json").write_text(
        json.dumps(
    ...
    return NuisanceDryRunResponse(
        ...
        functional_input_count=len(smooth_bolds),
        motion_parameter_count=len(motion_files),
        regressor_design_paths=regressor_paths,
        planned_output_paths=output_paths,
        warnings=warnings,
        next_actions=["Review regressor designs.", "Enable execution for nuisance regression."],
        safety_flags=nuisance_safety_flags(),
    )
