...
def _allowed(
    audit_id: str, record: dict, path: str, submitted: bool = False
) -> ModelAuditContractResult:
    return ModelAuditContractResult(
        ok=True,
        status="MODEL_AUDIT_ALLOWED",
        audit_id=audit_id,
        audit_record=record,
        audit_path=path,
    ...
    if isinstance(obj, dict):
        ...
        for i, item in enumerate(obj):
            if isinstance(item, dict | list):
                found.extend(_scan_forbidden(item, f"{prefix}[{i}]." if prefix else ""))
    return found
...
def validate_and_build_model_audit_record(
    *,
    ...
    if not audit_id or not isinstance(audit_id, str):
        return _blocked(_ERROR_CODES["audit_schema_invalid"], "audit_id required.")
    aid = audit_id.strip()
    if not _is_id_safe(aid):
        return _blocked(
            _ERROR_CODES["audit_output_id_invalid"],
            f"audit_id '{aid}' has invalid chars.",
            audit_id=aid,
        )
    if not run_id or not _is_id_safe(run_id):
        return _blocked(
            _ERROR_CODES["audit_run_id_invalid"], "run_id missing or invalid.", audit_id=aid
        )
    if not session_id or not _is_id_safe(session_id):
        return _blocked(
            _ERROR_CODES["audit_run_id_invalid"], "session_id missing or invalid.", audit_id=aid
        )
    if not event_type or event_type not in _ALLOWED_EVENT_TYPES:
        return _blocked(
            _ERROR_CODES["audit_event_unknown"],
            f"event_type '{event_type}' not allowed.",
            audit_id=aid,
        )
    ...
    if moid and not _is_id_safe(moid):
        return _blocked(
            _ERROR_CODES["audit_output_id_invalid"],
            f"model_output_id '{moid}' invalid chars.",
            audit_id=aid,
        )
    ...
    if root not in _ALLOWED_AUDIT_ROOTS:
        if not _is_path_safe(root):
            return _blocked(
                _ERROR_CODES["audit_path_blocked"], f"Unsafe audit root: '{root}'.", audit_id=aid
            )
        return _blocked(
            _ERROR_CODES["audit_path_blocked"],
            f"Audit root '{root}' not allowlisted.",
            audit_id=aid,
        )
    if not _is_path_safe(root):
        return _blocked(
    ...
    if ret < 1 or ret > 30:
        return _blocked(
            _ERROR_CODES["audit_retention_invalid"],
            f"retention_days must be 1-30, got {ret}.",
            audit_id=aid,
        )
    ...
    if forbidden:
        return _blocked(
            _ERROR_CODES["audit_forbidden_field"],
    ...
    if prov.get("inference_allowed") is True:
        return _blocked(
            _ERROR_CODES["provider_permission_blocked"],
            "inference_allowed must be false.",
            audit_id=aid,
        )
    if prov.get("model_loaded") is True:
        return _blocked(
            _ERROR_CODES["provider_permission_blocked"], "model_loaded must be false.", audit_id=aid
        )
    if prov.get("network_accessed") is True:
        return _blocked(
            _ERROR_CODES["provider_permission_blocked"],
            "network_accessed must be false.",
            audit_id=aid,
        )
    ...
    if adp.get("provider_call_allowed_by_adapter") is True:
        return _blocked(
            _ERROR_CODES["provider_permission_blocked"],
            "adapter provider_call_allowed_by_adapter must be false.",
            audit_id=aid,
        )
    ...
    if grd.get("provider_call_allowed_by_guard") is True:
        if grd.get("guard_status") != "GUI_GUARD_OK":
            return _blocked(
                _ERROR_CODES["guard_permission_invalid"],
                "guard should not grant provider permission for blocked status.",
                audit_id=aid,
            )

    sf = safety_flags or {}
    _flag_map = {
        "desktop_touched": "desktop_touched",
        "screenshot_captured": "screenshot_captured",
        "clipboard_accessed": "clipboard_accessed",
        "mouse_used": "mouse_used",
        "keyboard_used": "keyboard_used",
    }
    for key, _attr in _flag_map.items():
        if sf.get(key) is True:
            return _blocked(
                _ERROR_CODES["safety_flag_invalid"], f"{key} must be false.", audit_id=aid
            )
    ...
    if extra:
        ...
        for key in (
            ...
            if extra.get(key) is True:
                return _blocked(
                    _ERROR_CODES["extra_permission_blocked"],
                    f"Extra permission '{key}=true' blocked.",
                    audit_id=aid,
                )
        if extra.get("audit_root") and not _is_path_safe(str(extra["audit_root"])):
            return _blocked(
                _ERROR_CODES["audit_path_blocked"], "User-provided audit_root unsafe.", audit_id=aid
            )
        if extra.get("user_filename"):
            return _blocked(
                _ERROR_CODES["audit_path_blocked"],
                "user_filename must not be provided.",
                audit_id=aid,
            )
