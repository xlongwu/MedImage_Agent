...
export function RsfmriReportValidatorPanel({ baseUrl }: Props) {
  ...
  return (
    <div>
      <div className="row">
        <button onClick={handleRun}>Validate Latest Package</button>
        <button onClick={loadLatest}>Load Latest Validation</button>
        <button onClick={loadList}>List Validations</button>
      ...
      <div className="metricGrid">
        <div className="metricCard">
          <span>Status</span>
          <strong>{String(vr?.validation_status ?? "-")}</strong>
        </div>
        <div className="metricCard">
          <span>Checksum Mismatches</span>
          <strong>{String(st?.checksum_mismatch_total ?? "-")}</strong>
        </div>
        <div className="metricCard">
          <span>Missing Files</span>
          <strong>{String(st?.missing_files_total ?? "-")}</strong>
        </div>
        <div className="metricCard">
          <span>ZIP Test OK</span>
          <strong>{String(st?.zip_test_ok ?? "-")}</strong>
        </div>
        <div className="metricCard">
          <span>Safety Violations</span>
          <strong>{String(st?.safety_violations_total ?? "-")}</strong>
        </div>
      </div>
