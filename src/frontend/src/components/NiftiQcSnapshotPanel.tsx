...
export default function NiftiQcSnapshotPanel({ baseUrl, projectId }: Props) {
  ...
  if (!projectId) return <Sec><H3>NIfTI QC Snapshot</H3><div className="empty">Select a project.</div></Sec>;
  ...
  return (
    <Sec>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "grid", gap: 6, maxHeight: 360, overflow: "auto" }}>
            {data.images.map((img) => {
              ...
              return (
                <div key={key} style={{ padding: 8, border: "1px solid rgba(137,150,171,0.22)", borderRadius: 6, background: "#fff", display: "grid", gap: 4, fontSize: 11 }}>
                  {/* Summary row */}
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    {img.voxel_spacing?.length > 0 && <span style={mono}>{img.voxel_spacing.slice(0, 3).map(v => v?.toFixed(1)).join("/")}mm</span>}
                    {img.volume_count != null && <span>{img.volume_count}v</span>}
                    <span style={{ ...pill, background: img.readable ? "#e8f5e9" : "#ffebee", color: img.readable ? "#176b3b" : "#b53b3b", borderColor: img.readable ? "rgba(33,150,83,0.24)" : "rgba(235,87,87,0.26)" }}>{img.readable ? "✓" : "✗"}</span>
                    {img.warnings.length > 0 && <span style={{ color: "#9a5a15" }}>⚠{img.warnings.length}</span>}
                  </div>

                  {/* Path */}
                  <div style={mono}>{img.relative_path || img.path}</div>

                  <button onClick={() => setExpanded(p => { const n = new Set(p); isExpanded ? n.delete(key) : n.add(key); return n; })} style={{ fontSize: 11, fontWeight: 600 }}>
                  ...
                  {isExpanded && (
                    <div style={{ display: "grid", gap: 3, marginTop: 4 }}>
                      ...
                      <ThumbnailLoader effectiveBase={effectiveBase} projectId={projectId} imageId={img.image_id} />
                      {img.path !== img.relative_path && <div><b>path:</b> <span style={mono}>{img.path}</span></div>}
                      {img.dtype && <div><b>dtype:</b> {img.dtype}</div>}
                      {img.orientation && <div><b>orientation:</b> {img.orientation}</div>}
                      {img.affine_determinant != null && <div><b>affine det:</b> {img.affine_determinant.toFixed(4)}</div>}
                      {img.ndim != null && <div><b>ndim:</b> {img.ndim}</div>}

                      {/* Intensity stats */}
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 2, marginTop: 2 }}>
                        <span><b>min:</b> {img.intensity_min != null ? img.intensity_min.toFixed(2) : "—"}</span>
                        <span><b>max:</b> {img.intensity_max != null ? img.intensity_max.toFixed(2) : "—"}</span>
                        <span><b>mean:</b> {img.intensity_mean != null ? img.intensity_mean.toFixed(2) : "—"}</span>
                        <span><b>std:</b> {img.intensity_std != null ? img.intensity_std.toFixed(2) : "—"}</span>
                        <span><b>zero%:</b> {img.zero_fraction != null ? (100 * img.zero_fraction).toFixed(1) : "—"}</span>
                        <span><b>NaN:</b> {img.nan_count}</span>
                      </div>

                      {img.warnings.length > 0 && (
                        <div style={{ marginTop: 4, padding: 4, border: "1px solid rgba(242,153,74,0.24)", borderRadius: 4, background: "rgba(255,251,242,0.94)", color: "#9a5a15", fontSize: 10 }}>
                          {img.warnings.map((w, i) => <div key={i}>{w}</div>)}
                        </div>
                      )}
                    </div>
                  )}
      ...
      {data.errors.length > 0 && <div className="errorBox" style={{ marginBottom: 8 }}>{data.errors.join("\n")}</div>}
...
function ThumbnailLoader({ effectiveBase, projectId, imageId }: { effectiveBase: string; projectId: string; imageId: string }) {
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 4 }}>
        {thumbs.map((t, i) => (
          <div key={i} style={{ textAlign: "center", fontSize: 9, color: "#667085" }}>
            <img src={`data:image/png;base64,${t.png_base64}`} alt={String(t.view)} style={{ width: "auto", height: 80, borderRadius: 3, border: "1px solid rgba(137,150,171,0.2)" }} />
            <div>{String(t.view)} s{Number(t.slice_index)}</div>
          </div>
        ))}
        {thumbs.length === 0 && <span style={{ fontSize: 10, color: "#9a5a15" }}>No thumbnails available.</span>}
      </div>
    );
  }

  if (loading) return <button disabled style={{ fontSize: 11 }}>Loading slices...</button>;
  ...
  return (
    <button onClick={async () => {
      ...
      try {
        const d = await getProjectNiftiThumbnail(effectiveBase, projectId, imageId, { view: "all" });
