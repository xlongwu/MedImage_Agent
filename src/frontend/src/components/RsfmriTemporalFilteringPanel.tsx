...
import { getRsfmriTemporalFiltering, runRsfmriTemporalFiltering } from "../lib/api/legacy";
...
export function RsfmriTemporalFilteringPanel({ baseUrl }: Props) {
  ...
  async function handleRun() {
    if (
      !window.confirm(
        "Confirm to run Python Temporal Filtering? This only processes synthetic derivatives and will not modify rawdata or execute DPABI.",
      )
    )
      return;
    setStatus("RUNNING");
    ...
    <div>
      <div className="row">
        <button className="dangerButton" onClick={handleRun}>
          Approve and Run Python Temporal Filtering
        </button>
        <button onClick={handleLoad}>Load Temporal Filtering Results</button>
      ...
      <JsonBlock
        ...
        emptyText="No temporal filtering QC summary available"
      />
      <h3>Subject Temporal Filtering QC</h3>
      <JsonBlock
        value={loaded?.subject_temporal_filtering_qc}
        emptyText="No subject temporal filtering QC available"
      />
      ...
      <TextViewer
        ...
        emptyText="No temporal filtering QC report available"
      />
    </div>
  );
}
