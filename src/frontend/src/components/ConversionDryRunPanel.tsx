...
const confidenceColor: Record<string, string> = {
  high: "#176b3b", medium: "#9a5a15", low: "#b53b3b", manual_required: "#667085",
};
...
export default function ConversionDryRunPanel({ baseUrl, projectId }: Props) {
  ...
  if (!projectId) return <Sect><H3>Conversion Dry-Run</H3><div className="empty">Select a project.</div></Sect>;
  if (error) return <Sect><H3>Conversion Dry-Run</H3><div className="errorBox">{error}</div></Sect>;
  ...
  return (
    <Sect>
      ...
      {loading && <div className="empty" style={{ marginBottom: 12 }}>Generating conversion plan...</div>}
      {!data && !loading && <div className="empty" style={{ marginBottom: 12 }}>Click the button above to generate a conversion dry-run plan.</div>}
  ...
  return (
    <div style={{ display: "grid", gap: 4, padding: 8, border: "1px solid rgba(137, 150, 171, 0.22)", borderRadius: 6, background: "#fff" }}>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
        <span style={{ ...pill, background: "#eef1f6", color: "#667085", borderColor: "rgba(137, 150, 171, 0.28)" }}>{source.source_type}</span>
        <strong style={{ fontSize: 12 }}>{source.source_id}</strong>
        <span style={{ fontSize: 11, color: source.exists ? "#176b3b" : "#b53b3b" }}>{source.exists ? "exists" : "missing"}</span>
      </div>
      <div style={mono}>{source.root}</div>
      <div style={{ fontSize: 11, color: "#667085" }}>{source.file_count} file(s), {source.series_count} series, {source.subject_candidates.length} subject candidate(s)</div>
...
function MappingRow({ mapping }: { mapping: ConversionMappingPreview }) {
  return (
    <div style={{ display: "grid", gap: 4, padding: 8, border: "1px solid rgba(137, 150, 171, 0.22)", borderRadius: 6, background: "#fff" }}>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
        <span style={{ ...pill, background: "#eff6ff", color: "#2450a6", borderColor: "rgba(56, 103, 214, 0.24)" }}>{mapping.source_type}</span>
        <span style={{ fontSize: 12, fontWeight: 700, color: confidenceColor[mapping.confidence] }}>{mapping.confidence}</span>
        {mapping.subject_id && <span style={{ fontSize: 11, color: "#667085" }}>{mapping.subject_id}</span>}
        {mapping.modality && <span style={{ ...pill, background: "#eef1f6", color: "#667085", borderColor: "rgba(137, 150, 171, 0.28)" }}>{mapping.modality}</span>}
        {mapping.suffix && <span style={{ ...pill, ...suffixPill(mapping.suffix) }}>{mapping.suffix}</span>}
      </div>
      {mapping.suggested_relative_path && <div style={mono}>{mapping.suggested_relative_path}</div>}
      {mapping.source_path && <div style={{ ...mono, color: "#98a2b3" }}>source: {mapping.source_path}</div>}
...
function suffixPill(suffix: string): React.CSSProperties {
  if (suffix === "bold") return { background: "#e8f5e9", color: "#176b3b", borderColor: "rgba(33, 150, 83, 0.24)" };
  if (suffix === "T1w" || suffix === "T2w" || suffix === "FLAIR") return { background: "#eff6ff", color: "#2450a6", borderColor: "rgba(56, 103, 214, 0.24)" };
