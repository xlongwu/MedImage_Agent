  ...
  return (
    <Sec>
      ...
      <div className={styles.style002}>
        ...
        <select
          value={cacheMode}
          onChange={(e) => setCacheMode(e.target.value as "off" | "prefer" | "refresh")}
        ...
        <button
          onClick={async () => {
            ...
            setFpLoading(true);
            try {
              setFpData(await getQcDashboardFingerprint(effectiveBase, projectId));
            } catch {
              setFpData(null);
            } finally {
              setFpLoading(false);
        ...
        <>
          ...
          {data.cache && (
            <div className={styles.style005}>
              <div
                style={{
                  color: "#667085",
                  marginBottom: data.cache.module_records?.length ? 4 : 0,
                }}
              >
                Cache: <b>{data.cache.mode}</b>
                {data.cache.mode !== "off" &&
                  ` · modules: ${Object.values(data.cache.module_hits ?? {}).filter(Boolean).length}/${Object.keys(data.cache.module_hits ?? {}).length || Object.keys(data.cache.module_records ?? []).length || 1}`}
              </div>
                  ...
                  {data.cache.module_records.map((r, i) => {
                    ...
                    return (
                      <div key={i} className={styles.style007}>
                              ...
                              r.status === "hit"
                                ...
                                : r.status === "stale"
                                  ...
                                  : r.status === "error"
                                    ? "#ffebee"
                                    : "#eef1f6",
                            color: statusColors[r.status] || "#667085",
                        ...
                        {r.module_version && (
                          <span className={styles.style011}>v{r.module_version}</span>
                        )}
                        {r.warnings.length > 0 && (
                          <span className={styles.style012}>⚠{r.warnings.length}</span>
                        )}
                        {r.errors.length > 0 && (
                          <span className={styles.style013}>✗{r.errors.length}</span>
                        )}
                      </div>
                    );
                  })}
              ...
              {data.cache.mode !== "off" && (
                <div className={styles.style014}>
                  Module cache currently applies only to NIfTI QC Snapshot. Other modules run
                  normally.
                </div>
              )}
              {(data.cache.cache_warnings?.length ?? 0) > 0 && (
                <div className={styles.style015}>
                  ⚠ {data.cache.cache_warnings![0].slice(0, 80)}
                </div>
              )}
            </div>
          )}
          ...
          {fpData && (
            <div className={styles.style016}>
              ...
              <div className={styles.style018}>
                ...
                <span>
                  <b>files:</b> {fpData.fingerprint.file_count}
                </span>
                <span>
                  <b>size:</b> {fpData.fingerprint.total_size_bytes}
                </span>
                <span>
                  <b>mtime:</b> {fpData.fingerprint.newest_mtime_iso ?? "—"}
                </span>
                <span>
                  <b>trunc:</b> {String(fpData.fingerprint.truncated)}
                </span>
              </div>
              <div className={styles.style019}>
                Fingerprint is metadata-only and used for future cache invalidation. No cache is
          ...
          <div className={styles.style020}>
            ...
            <MetricTile
              ...
              tone={data.blocked_count > 0 ? "red" : "neutral"}
            />
          ...
          {data.modules.length > 0 && (
            <CollapsibleDetails
              title="QC module details"
              summary={`${data.modules.length} module(s)`}
            >
                  ...
                  <div key={m.module_id} className={styles.style022}>
                    ...
                    {Object.keys(m.key_metrics).length > 0 && (
                      <div className={styles.style025}>
                        {Object.entries(m.key_metrics)
                          .slice(0, 6)
                          .map(([k, v]) => (
                            <span key={k}>
                              {k}: <b>{v == null ? "—" : String(v)}</b>
                            </span>
                          ))}
                      </div>
                    )}
                    <div className={styles.style026}>
                      {m.warnings.length > 0 && (
                        <span className={styles.style027}>⚠{m.warnings.length}</span>
                      )}
                      {m.errors.length > 0 && (
                        <span className={styles.style028}>✗{m.errors.length}</span>
                      )}
                      {m.next_actions.length > 0 && (
                        <span className={styles.style029}>→ {m.next_actions[0].slice(0, 60)}</span>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {data.artifacts.length > 0 && (
            <CollapsibleDetails
              ...
              summary={`${data.artifacts.length} artifact(s)`}
            >
          ...
          {data.report_markdown && (
            <div className={styles.style031}>
              <button onClick={() => setShowMd(!showMd)} style={{ fontWeight: 600 }}>
          ...
          {data.overall_errors.length > 0 && (
            <div className={`errorBox ${styles.style039}`}>{data.overall_errors.join("\n")}</div>
          )}
...
const H3: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <h3 className={styles.style037}>{children}</h3>
);
const Sub: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <span className={styles.style038}>{children}</span>
);
