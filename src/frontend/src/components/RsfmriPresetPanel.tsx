...
export default function RsfmriPresetPanel({ baseUrl, projectId, onReviewDraft }: Props) {
  ...
  useEffect(() => {
    ...
    getPipelinePreset(effectiveBase, "rsfmri_preproc_mvp")
      .then((r) => {
        if (id === reqRef.current) setPreset(r.preset);
      })
      .catch((e) => {
    ...
    <Sec>
      ...
      >
        <div>
          <H3>{preset.name}</H3>
          <Sub>
            {preset.modality} · v{preset.version}
          </Sub>
        </div>
      </div>
      <div
          ...
          >
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <span
                style={{
                  ...pill,
                  background: n.executable ? "#e8f5e9" : "#fff7ed",
                  color: n.executable ? "#176b3b" : "#9a5a15",
                  borderColor: n.executable
                    ? "rgba(33, 150, 83, 0.24)"
      ...
      {result && (
        <div
          style={{
            border: `1px solid ${result.ok ? "rgba(33, 150, 83, 0.24)" : "rgba(235, 87, 87, 0.24)"}`,
            borderRadius: 6,
            padding: 12,
            marginBottom: 12,
          }}
