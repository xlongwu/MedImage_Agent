...
export default function RsfmriQcPlanningReportPanel({ baseUrl, projectId }: Props) {
  ...
  if (!projectId) return <Sec><H3>QC Planning Report</H3><div className="empty">Select a project.</div></Sec>;
  if (error) return <Sec><H3>QC Planning Report</H3><div className="errorBox">{error}</div></Sec>;

  return (
    <Sec>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div><H3>rs-fMRI QC Planning Report</H3><Sub>Generate a combined BOLD reference + motion QC planning artifact.</Sub></div>
          ...
          {data.artifacts.length > 0 && (
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              {data.artifacts.map((a, i) => (
                <span key={i} style={{ ...pill, background: a.exists ? "#e8f5e9" : "#ffebee", color: a.exists ? "#176b3b" : "#b53b3b", borderColor: a.exists ? "rgba(33, 150, 83, 0.24)" : "rgba(235, 87, 87, 0.26)" }}>
