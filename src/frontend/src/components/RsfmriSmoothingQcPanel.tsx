...
export function RsfmriSmoothingQcPanel({ baseUrl }: Props) {
  ...
  async function handleRun() {
    const confirmed = window.confirm(
      "Confirm to run SPM Smoothing + Smoothing QC? This only processes synthetic BIDS data and will not modify rawdata.",
    );
    if (!confirmed) return;
    setStatus("RUNNING");
    setError("");
    ...
    <div>
      <div className="row">
        <button className="dangerButton" onClick={handleRun}>
          Approve and Run Smoothing + Smoothing QC
        </button>
