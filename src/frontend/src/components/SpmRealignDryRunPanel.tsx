...
export default function SpmRealignDryRunPanel({ baseUrl, projectId }: Props) {
  ...
  return (
    <Sec>
      ...
      >
        Dry-run only. MATLAB/SPM is not executed. No files or directories are created. Predicted
        output paths are previews, not real outputs. Research-use only. Not for clinical use.
      </div>
        ...
        <>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
            <span
              style={{
                ...pill,
                background: data.execution_enabled ? "#e8f5e9" : "#ffebee",
                color: data.execution_enabled ? "#176b3b" : "#b53b3b",
                borderColor: data.execution_enabled
                  ? "rgba(33,150,83,0.24)"
                  : "rgba(235,87,87,0.26)",
              }}
            >
              exec: {String(data.execution_enabled)}
            </span>
            <span
              style={{
                ...pill,
            ...
            >
              approval: {String(data.approval_required)}
            </span>
            <span
              style={{
                ...pill,
            ...
            >
              audit: {String(data.audit_required)}
            </span>
          </div>
          ...
          {data.output_root_preview && (
            <div style={{ ...mono, marginBottom: 8 }}>output: {data.output_root_preview}</div>
          )}
          ...
          {data.param_errors.length > 0 && (
            <div className="errorBox" style={{ marginBottom: 8 }}>
              {data.param_errors.join("\n")}
            </div>
          )}
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...pill,
                            background: inp.valid_for_realign ? "#e8f5e9" : "#ffebee",
                            color: inp.valid_for_realign ? "#176b3b" : "#b53b3b",
                            borderColor: inp.valid_for_realign
                              ? "rgba(33,150,83,0.24)"
                              : "rgba(235,87,87,0.26)",
                          }}
                        >
                          {inp.valid_for_realign ? "valid" : "invalid"}
                        </span>
                      ...
                      >
                        {expanded.has(key) ? "Hide" : "Show"} outputs
                      </button>
                      {expanded.has(key) && (
                          ...
                          {inp.predicted_outputs.map((o, j) => (
                            ...
                            >
                              <span
                                style={{
                                  ...pill,
                                  background: o.exists ? "#fff7ed" : "#eef1f6",
                                  color: o.exists ? "#9a5a15" : "#667085",
