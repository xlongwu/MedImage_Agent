...
export default function SpmRealignWrapperSkeletonPanel({ baseUrl, projectId }: Props) {
  ...
  if (!projectId) return <Sec><H3>SPM Wrapper Skeleton</H3><div className="empty">Select a project.</div></Sec>;
  if (error) return <Sec><H3>SPM Wrapper Skeleton</H3><div className="errorBox">{error}</div></Sec>;

  return (
    <Sec>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div><H3>SPM Wrapper Skeleton</H3><Sub>Preview MATLAB batch and provenance without execution.</Sub></div>
      ...
      {data && (
        <>
          <div style={{ marginBottom: 8, fontSize: 13, fontWeight: 700 }}>Command template: <code style={mono}>{data.command_template_id}</code></div>
          ...
          {data.provenance_preview && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, marginBottom: 12, fontSize: 11, color: "#667085" }}>
              <span>inputs: <b>{data.provenance_preview.input_count}</b></span>
              <span>outputs: <b>{data.provenance_preview.predicted_output_count}</b></span>
              <span>exec enabled: <b style={{ color: data.provenance_preview.execution_enabled ? "#176b3b" : "#b53b3b" }}>{String(data.provenance_preview.execution_enabled)}</b></span>
              <span>safe allowlist: <b style={{ color: data.provenance_preview.safe_allowlist_enabled ? "#176b3b" : "#b53b3b" }}>{String(data.provenance_preview.safe_allowlist_enabled)}</b></span>
            </div>
          )}

          {data.manifest_summary && Object.keys(data.manifest_summary).length > 0 && (
            <div style={{ marginBottom: 12, padding: 10, border: "1px solid rgba(137,150,171,0.22)", borderRadius: 6, background: "rgba(249,249,251,0.9)" }}>
              <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 6 }}>Output Manifest Preview</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, fontSize: 11, color: "#667085", marginBottom: 4 }}>
                <span>manifests: <b>{String(data.manifest_summary.manifest_count ?? 0)}</b></span>
                <span>items: <b>{String(data.manifest_summary.total_items ?? 0)}</b></span>
                <span>overwrites: <b style={{ color: (data.manifest_summary.would_overwrite_count as number) > 0 ? "#b53b3b" : "#667085" }}>{String(data.manifest_summary.would_overwrite_count ?? 0)}</b></span>
                <span>missing req: <b style={{ color: "#b53b3b" }}>{String(data.manifest_summary.missing_required_count ?? 0)}</b></span>
                <span>verified: <b>{String(data.manifest_summary.verified_count ?? 0)}</b></span>
              </div>
              <div style={{ fontSize: 10, color: "#9a5a15" }}>
                These manifests are preview-only. Missing outputs are expected because MATLAB/SPM has not been executed.
              </div>
            </div>
          )}
          ...
          {data.matlab_batch_preview && (
            <div style={{ marginBottom: 12 }}>
              <button onClick={() => setShowBatch(!showBatch)} style={{ fontWeight: 600 }}>{showBatch ? "Hide" : "Show"} MATLAB Batch Preview</button>
              {showBatch && <pre style={{ marginTop: 8, padding: 12, border: "1px solid rgba(137,150,171,0.24)", borderRadius: 6, background: "#0f172a", color: "#e5e7eb", fontSize: 11, maxHeight: 360, overflow: "auto", whiteSpace: "pre-wrap", lineHeight: 1.5 }}>{data.matlab_batch_preview}</pre>}
            </div>
          )}
