...
import type { InsightsDashboard, InsightErrorCategory, InsightFailedNode, InsightNodeTiming, InsightTrendPoint } from '../lib/api/insights';
          ...
          {insights.recent_trend && insights.recent_trend.length > 0 && (
            <div className={styles.section}>
              ...
              <div className={styles.trendBars}>
                {insights.recent_trend.slice().reverse().map((r: InsightTrendPoint, i: number) => (
                  <div key={i} title={`${r.run_id}: ${r.status}`} className={styles.trendBar} style={{
          ...
          <div className={styles.dataGrid}>
            {insights.slowest_nodes && insights.slowest_nodes.length > 0 && (
              <div>
                ...
                <table className={styles.dataTable}>
                  <thead><tr className={styles.tableHead}>{['Node', 'Avg (s)', 'Count', 'Fail %'].map(h => <th key={h} className={styles.tableHeaderCell}>{h}</th>)}</tr></thead>
                  <tbody>
                    {insights.slowest_nodes.map((n: InsightNodeTiming) => (
                      <tr key={n.node_id} className={styles.tableRow}>
                        ...
                        <td className={`${styles.tableCell} ${n.failure_rate > 20 ? styles.dangerCell : ''}`}>{n.failure_rate}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {insights.most_failed_nodes && insights.most_failed_nodes.length > 0 && (
              <div>
                ...
                <table className={styles.dataTable}>
                  <thead><tr className={styles.tableHead}>{['Node', 'Failed', 'Total', 'Fail %'].map(h => <th key={h} className={styles.tableHeaderCell}>{h}</th>)}</tr></thead>
                  <tbody>
            ...
            {insights.top_error_categories && insights.top_error_categories.length > 0 && (
              <div>
                ...
                <table className={styles.dataTable}>
                  <thead><tr className={styles.tableHead}>{['Category', 'Count'].map(h => <th key={h} className={styles.tableHeaderCell}>{h}</th>)}</tr></thead>
                  <tbody>
...
function KpiCard({ label, value, color }: { label: string; value: ReactNode; color?: string }) {
  return (
    <div className={styles.kpiCard}>
      ...
      <div className={styles.kpiValue} style={{ color: color || '#333' }}>{value}</div>
    </div>
  );
}
