...
export function RsfmriGroupSummaryPanel({ baseUrl }: Props) {
  ...
  return (
    <div>
      <div className="row">
        <button onClick={handleRun}>Generate Group Summary</button>
        <button onClick={handleLoad}>Load Dashboard</button>
      ...
      <JsonBlock value={loaded?.dataset_summary} emptyText="No summary" />
      <h3>Dashboard Data</h3>
      <JsonBlock value={loaded?.dashboard_data} emptyText="No dashboard" />
      <h3>Pipeline Completeness</h3>
      <JsonBlock value={loaded?.pipeline_completeness} emptyText="No completeness" />
      <h3>Contracts Overview</h3>
      <JsonBlock value={loaded?.contracts_overview} emptyText="No contracts" />
      <h3>Subject Metrics CSV</h3>
      <JsonBlock value={{ path: loaded?.subject_metrics_table_path }} emptyText="No CSV" />
      <h3>Report</h3>
      <TextViewer
        text={
