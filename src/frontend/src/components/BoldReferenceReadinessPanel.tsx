...
export default function BoldReferenceReadinessPanel({ baseUrl, projectId }: Props) {
  ...
  if (!projectId) return <Sec><H3>BOLD Reference Readiness</H3><div className="empty">Select a project.</div></Sec>;
  if (loading) return <Sec><H3>BOLD Reference Readiness</H3><div className="empty">Inspecting...</div></Sec>;
  if (error) return <Sec><H3>BOLD Reference Readiness</H3><div className="errorBox">{error}</div></Sec>;
  if (!data) return null;

  return (
    <Sec>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div><H3>BOLD Reference Readiness</H3><Sub>Inspect BOLD NIfTI files and propose reference strategy.</Sub></div>
  ...
  return (
    <div style={{ display: "grid", gap: 4, padding: 8, border: "1px solid rgba(137, 150, 171, 0.22)", borderRadius: 6, background: "#fff" }}>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <span style={{ ...pill, background: candidate.is_4d ? "#e8f5e9" : "#fff7ed", color: candidate.is_4d ? "#176b3b" : "#9a5a15", borderColor: candidate.is_4d ? "rgba(33, 150, 83, 0.24)" : "rgba(242, 153, 74, 0.28)" }}>{candidate.is_4d ? "4D" : "3D"}</span>
        <span style={{ fontSize: 11, color: "#667085" }}>{candidate.volume_count ?? "?"} vol(s)</span>
        {candidate.dimensions.length > 0 && <span style={{ fontSize: 11, color: "#667085" }}>{candidate.dimensions.join("×")}</span>}
        <span style={{ ...pill, background: candidate.has_sidecar ? "#e8f5e9" : "#ffebee", color: candidate.has_sidecar ? "#176b3b" : "#b53b3b", borderColor: candidate.has_sidecar ? "rgba(33, 150, 83, 0.24)" : "rgba(235, 87, 87, 0.26)" }}>sidecar: {String(candidate.has_sidecar)}</span>
        <span style={{ ...pill, background: candidate.reference_strategy === "manual_required" ? "#ffebee" : "#e8f5e9", color: candidate.reference_strategy === "manual_required" ? "#b53b3b" : "#176b3b", borderColor: candidate.reference_strategy === "manual_required" ? "rgba(235, 87, 87, 0.26)" : "rgba(33, 150, 83, 0.24)" }}>{candidate.reference_strategy.replace(/_/g, " ")}</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "2px 8px", fontSize: 11, color: "#667085" }}>
        {candidate.repetition_time != null && <div><span>TR: </span><b>{candidate.repetition_time}s</b></div>}
        {candidate.task_name && <div><span>Task: </span><b>{candidate.task_name}</b></div>}
        {candidate.has_slice_timing && <div><b>SliceTiming ✓</b></div>}
        {candidate.phase_encoding_direction && <div><span>PE: </span><b>{candidate.phase_encoding_direction}</b></div>}
      </div>
