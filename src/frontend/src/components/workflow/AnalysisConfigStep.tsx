...
const items: { key: keyof AnalysisConfig; label: string; desc: string; dependsOn: keyof AnalysisConfig | null }[] = [
  { key: "enabled", label: "Run Post-Processing Analysis", desc: "Only preprocessing without derived metrics", dependsOn: null },
  { key: "alffFalff", label: "ALFF / fALFF", desc: "Amplitude of Low Frequency Fluctuations", dependsOn: "enabled" },
  { key: "reho", label: "ReHo", desc: "Regional Homogeneity (KCC, neighborhood 27)", dependsOn: "enabled" },
  { key: "functionalConnectivity", label: "Functional Connectivity", desc: "ROI-based correlation matrix (4 ROIs)", dependsOn: "enabled" },
  { key: "groupSummary", label: "Group Summary", desc: "Cross-subject aggregate statistics", dependsOn: "enabled" },
    ...
    <div>
      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.key} onClick={() => !disabled && toggle(item.key)} style={{
            ...
            border: "1px solid", borderColor: enabled && !disabled ? "#c8e6c9" : "#eee",
            borderRadius: 6, background: enabled && !disabled ? "#f1f8e9" : "#fafafa",
            opacity: disabled ? 0.4 : enabled ? 1 : 0.5,
          }}>
            <div style={{
              width: 22, height: 22, borderRadius: 4, marginRight: 12, flexShrink: 0,
              background: enabled && !disabled ? "#4caf50" : "#e0e0e0", color: "#fff",
            ...
            {item.key === "enabled" && <div style={{ fontSize: 12, color: "#1976d2", fontWeight: 600 }}>Main Toggle</div>}
          </div>
        );
      })}
      ...
      {state.analysis.alffFalff.enabled && state.analysis.enabled && (
        <div style={{ marginTop: 12, padding: 12, background: "#f5f5f5", borderRadius: 6, fontSize: 13 }}>
          <strong>ALFF/fALFF:</strong> Band {state.analysis.alffFalff.lowHz}-{state.analysis.alffFalff.highHz} Hz
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
        <button onClick={() => dispatch({ type: "SET_STEP", step: 2 })} style={btnBack}>Back</button>
        <button onClick={() => dispatch({ type: "SET_STEP", step: 4 })} style={btnNext}>Next: Confirm & Run</button>
      </div>
    </div>
  );
}
