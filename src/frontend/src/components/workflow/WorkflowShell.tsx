...
export default function WorkflowShell() {
  ...
  const renderStep = () => {
    switch (state.step) {
      case 0:
        return <IntroCard onStart={() => dispatch({ type: "SET_STEP", step: 1 })} />;
      case 1:
        return <DataUploadStep state={state} dispatch={dispatch} />;
      case 2:
        return <PreprocessingConfigStep state={state} dispatch={dispatch} />;
      case 3:
        return <AnalysisConfigStep state={state} dispatch={dispatch} />;
      case 4:
        return <RunConfirmStep state={state} dispatch={dispatch} />;
      case 5:
        return <ResultsOverviewStep state={state} dispatch={dispatch} />;
      default:
        return <IntroCard onStart={() => dispatch({ type: "SET_STEP", step: 1 })} />;
    }
  };
