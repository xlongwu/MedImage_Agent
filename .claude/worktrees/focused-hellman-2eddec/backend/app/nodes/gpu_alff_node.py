...
if TYPE_CHECKING:
    from backend.app.runtime.node_registry import NodeExecutionContext
...
def gpu_alff_subject_node(
    ...
    subject_id = subject_id or subject_record.get("subject_id", "unknown")

    # Find smoothed BOLD input
    ...
    prefer_gpu = gpu_config.get("prefer_gpu", True)
    require_gpu = gpu_config.get("require_gpu", False)
    benchmark_compare = gpu_config.get("benchmark_compare_cpu_gpu", True)
