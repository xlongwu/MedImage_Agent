...
def write_dpabi_temporal_filtering_contract(
    ...
    payload = {
        ...
        "planned_outputs": [
            "derivatives/rsfmri_preproc/{subject_id}/func/dpabi_filtered_resid_swr*.nii",
            "logs/{subject_id}_dpabi_temporal_filtering.log"
        ],
        "parameters": {
            "low_hz": 0.01,
            "high_hz": 0.08,
...
def main() -> int:
    ...
    if approved:
        pipeline = _make_approved_pipeline_copy(
            source=pipeline,
            target=Path("work/rsfmri/approved_pipeline_temporal_filtering.yaml"),
        )
