...
if (Cfg.IsOrganizefmriprepResults==1)
    ...
    if exist([Cfg.WorkingDir,filesep,'fmriprep',filesep,'sourcedata'],'dir')
        if isdeployed && (isunix && (~ismac)) % If running within docker with compiled version
            ...
            Command=sprintf('%s mv /data/fmriprep/sourcedata/freesurfer /data/freesurfer', CommandInit);
        end
        system(Command);
        ...
        if isdeployed && (isunix && (~ismac)) % If running within docker with compiled version
            ...
            Command=sprintf('%s rm -rf /data/fmriprep/sourcedata', CommandInit);
        end
        system(Command);
    ...
    if ~(isdeployed && (isunix && (~ismac))) % Give permission
        Command=sprintf('%s chmod -R 777 /data/fmriprep/', CommandInit);
        system(Command);
    end
