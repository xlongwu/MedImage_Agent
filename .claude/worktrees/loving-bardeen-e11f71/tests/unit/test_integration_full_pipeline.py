...
def test_synthetic_bids_to_alff_integration(tmp_path: Path):
    ...
    qd = d/"rsfmri_qc"/sid; qd.mkdir(parents=True)

    # Create synthetic filtered residual
    nt = 16
    ...
    (qd/"slice_timing_qc.json").write_text(json.dumps({"ok":True,"subject_id":sid,"tr":2.0,"slice_timing_status":"PASS"}), encoding="utf-8")
    # Temporal filtering QC
    (qd/"temporal_filtering_qc.json").write_text(json.dumps({"ok":True,"subject_id":sid,"tr":2.0,"low_hz":0.01,"high_hz":0.08,"filtering_qc_status":"PASS"}), encoding="utf-8")
    ...
    r_fc = run_python_functional_connectivity_subject(subject_id=sid, derivatives_dir=str(d), roi_count=2)
    ...
    r_cm = build_confound_matrix_for_subject(subject_id=sid, motion_parameter_file=str(mf), output_dir=str(d), model="friston24")
    ...
    (rpt/"rsfmri"/"group_summary"/"dataset_summary.json").write_text(json.dumps({"ok":True,"subjects_total":1,"stage_status_counts":{}}), encoding="utf-8")
    from backend.app.tools.group_dataset_summary import build_group_dataset_summary
    r_gs = build_group_dataset_summary(derivatives_dir=str(d), reports_dir=str(rpt), work_dir=str(tmp_path/"work"))
