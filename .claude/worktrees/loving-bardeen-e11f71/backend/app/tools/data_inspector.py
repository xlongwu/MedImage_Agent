...
def inspect_dataset(
    ...
    report_path.write_text(json.dumps(completeness_report, ensure_ascii=False, indent=2), encoding="utf-8")
    
    # Write subject table CSV
    with table_path.open("w", newline="", encoding="utf-8") as f:
        if subject_table_rows:
            writer = csv.DictWriter(f, fieldnames=["subject_id", "status", "t1w_exists", "bold_count", "issues"])
            writer.writeheader()
