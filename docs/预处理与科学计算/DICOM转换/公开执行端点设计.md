...
**Version:** v1.0-draft
**Date:** 2026-06-10
**Parent:** `docs/预处理与科学计算/DICOM转换/发布加固.md` (Phase 4K-0 / 4L-0)

> Historical design: this proposed public `dcm2niix` execution surface is not
> the current execution contract. The legacy public endpoint remains
> fail-closed; current conversion runs only through the in-project converter's
> reviewed gateway/readiness path.
...
- [ ] Update `docs/预处理与科学计算/DICOM转换/发布加固.md`
- [ ] Run full test suite
