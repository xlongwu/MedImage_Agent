...
现有代码可以视为“记忆骨架/历史兼容面”，不能宣称已经具备 Codex 式长期记忆。新系统应以当前 `SQLiteDesktopStore` 的项目事实为来源，以独立 memory SQLite 承载长期记忆，并用事务 outbox 连接两者；旧 `memory_store.py`、`SessionDB` 和 `/api/sessions/*` 只做受控迁移或兼容，不再承担新架构的权威职责。
...
| Context builder | `src/backend/app/services/memory_context_service.py` | 生成 typed、只读的低风险 constraints、science decision suggestions、来源 refs 与稳定 hash；不拼接自由文本 Prompt |
...
- `MemoryContextService` 是进入任务决策/规划链的唯一入口；其他 Service 不直接拼接 memory 文本。scientific memory 只进入当前任务 decision suggestion，不进入 planner 自由文本 constraints。
...
- `store_meta` 记录 memory schema version；迁移必须可重放、可回滚到只读兼容模式。
- `memory_context_hash` 复用 `planner.audit_record.stable_hash()`。所有时间先规范化为 UTC ISO 8601 字符串。hash payload 是完整 `MemoryContext` 去掉 `context_hash` 后的对象：`decision_suggestions` 按 `(decision_kind, memory_id, revision_hash)` 排序，`evidence_refs` 按 `(kind, memory_id, revision_hash, source_ref)` 排序，`planner_constraints` 使用 canonical JSON key 顺序；payload 必须包含 schema version、retrieval policy version、project ID 和 `omitted_count`，明确排除数据库 rowid 和未规范化运行时对象。
...
- `AgentTaskCommandService` 在调用 planner 前通过 `MemoryContextService` 读取 memory context。
...
- memory 变化后，已保存 Reviewed Plan 不变；若重新规划，即使 node/params 相同，只要 memory snapshot 不同也生成不同 plan hash/Reviewed Plan identity 和新 Approval Summary。SQLite 唯一键、schema、read model 和迁移必须同步。
- Execution Ticket 继续绑定 Reviewed Plan/plan hash；记忆不直接进入 runner，也不扩张 allowlist/write roots。
...
| `safety` | 否；通常不进入长期记忆 | 否 | 始终从静态治理、Tool Catalog、Gate 和实时配置读取 |
...
- 所有模型调用只发送已经清洗、最小化的结构化投影，并记录 provider/model/prompt version，不记录 API key。外部 provider 还需独立的用户 opt-in、数据驻留/处理说明和关闭后的确定性 fallback。
...
| GET | `/consent` | 读取安装级 availability 与项目级 generation/use consent、epoch 和 cutoff |
...
- 只读显示安装级 availability；提供项目级“生成记忆”和“使用记忆”两个 consent 开关，明确说明保存/使用范围。关闭时展示已有条目的保留、导出和忘记选择。
...
当前 source/release line 是 `v0.6.0-rc1`，处于 `v0.6.0-rc2` convergence/capability freeze。本文档可先合入；代码实施应在 RC2 完成后进行，或通过单独批准重新打开 capability/contract review。
...
| CREATE | `src/backend/app/services/memory_retrieval_service.py`、`memory_context_service.py` | scope/expiry/trust 前置过滤、FTS authoritative re-join、排序/预算和 canonical context hash；只输出 typed suggestions/refs |
...
| MODIFY | `src/backend/app/planner/reviewed_plan_store.py`、`src/backend/app/schemas/desktop.py`、`src/backend/app/services/mock_store.py` | `reviewed_plan_identity()` 把 canonical memory snapshot 纳入 plan hash；同步 record schema、SQLite uniqueness、snapshot JSON 与 legacy migration/lookup |
...
- 修改 Reviewed Plan identity，使 canonical memory snapshot 真实参与 plan hash；同步 SQLite 唯一键、Approval Summary、read model、Execution Ticket 兼容和前端 plan review。
...
- 模型失败时回退确定性流程；模型输出只生成 proposed candidate/diff。
...
| Candidate | transactional outbox、source commit before worker crash、ordered sequence、mutable source hash mismatch、consistent snapshot backfill boundary、dedupe、rejection advances watermark、LLM off fallback |
...
| 规划只有一个记忆入口且 scientific 不进 planner | 全仓搜索 MemoryContextService/`memory_context` 调用并检查 typed schema/guard |
