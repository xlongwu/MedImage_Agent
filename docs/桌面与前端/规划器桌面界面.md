...
> Historical baseline only. This document records the `v0.3` API/UI surface and
> must not be used as current routing or execution guidance. The current
> Agent-first source workflow, domain routers, Approval Summary, Execution
> Ticket, sole Execution Gateway, and desktop boundaries are documented in
> [`系统架构.md`](../架构与决策/系统架构.md),
> [`安全边界.md`](../安全与审批/安全边界.md), and
