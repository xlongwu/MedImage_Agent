...
3. 在 Goal 输入框输入: `test`
4. 点击 **Generate Plan**
5. 在 **Goal Contract Review** 区域检查候选 JSON；未确认时验证 dry-run 按钮不可用
6. 勾选 Goal Contract 人工审查确认并点击 **Review Goal Contract and Save**
7. 确认显示 **Goal Contract reviewed**，且 Active plan ID 已切换为新的不可变方案 ID
8. 在 Approval Gate 区域填写 approval 信息并点击 **Check Approval Gate**
9. 在 Dry-run 区域点击 **Dry-run Execution Check**
10. 确认 dry-run result 显示 **DRY_RUN_OK**
11. 向下滚动到 **Reviewed Execution** 区域
...
| 15 | 未审查 Goal Contract | 状态明确显示 review required；dry-run 与执行均 disabled |
| 16 | Goal Contract 已审查 | 请求包含原候选（或用户明确编辑后的候选）和 `reviewed_actor`；后端返回新的 `REVIEWED` plan ID |
| 17 | 后端返回 `REVIEWED_PLAN_NEEDS_GOAL_REVIEW` | 显示可操作的 Goal Contract 审查说明，不显示 Unknown status |
