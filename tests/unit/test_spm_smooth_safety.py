...
def test_matlab_with_args_blocked(tmp_path):
    ...
    assert result["ok"] is False; assert result.get("stage") == "matlab_safety_preflight"


def test_safety_error_no_subprocess(monkeypatch, tmp_path):
    _make_env(tmp_path); called = []
...
def test_missing_func_blocked(tmp_path):
    ...
    assert result["ok"] is False; assert "functional" in str(result["errors"]).lower()
