...
def test_python_reho_outputs_metric_map(tmp_path: Path):
    ...
    nt = 8
    base = np.linspace(0, 1, nt, dtype=np.float32)
    ...
    result = run_python_reho_subject(
        subject_id=sid, derivatives_dir=str(d), neighborhood=27, use_gm_mask=False
    )
    assert result["ok"] is True
    assert (d / "rsfmri_metrics" / sid / "reho.nii").exists()
    qp = d / "rsfmri_qc" / sid / "reho_qc.json"
    assert qp.exists()
    pl = json.loads(qp.read_text(encoding="utf-8"))
    assert pl["subject_id"] == sid
    assert pl["reho_qc_status"] in {"PASS", "WARNING"}
    assert pl["valid_voxel_count"] > 0
    assert 0 <= pl["reho_mean"] <= 1
...
def test_python_reho_writer_uses_canonical_compute_backend(tmp_path: Path, monkeypatch):
    ...
    def fake_compute(
        data_4d, neighborhood=27, gm_mask=None, prefer_gpu=True, require_gpu=False, **_kwargs
    ):
