...
def _make_metric_exec(tmp_path, exec_id="ar-ex-maps"):
    ...
    (ed / "manifest.json").write_text('{"status":"succeeded","alff":{"computed":true},"reho":{"computed":true}}')
...
def test_registers_bids_like_metric_maps_in_artifact_registry(tmp_path, monkeypatch):
    _setup(tmp_path, monkeypatch); _make_metric_exec(tmp_path)
    result = register_alff_reho_outputs("brain-tumor-study", "pp-test",
        StageOutputRegistrationRequest(execution_id="ar-ex-maps", confirm_sandbox_outputs=True), project_dir=str(tmp_path))
    assert result.ok
