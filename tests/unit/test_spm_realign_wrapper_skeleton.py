...
from src.backend.app.services import bold_reference_readiness, motion_qc_readiness, spm_realign_dry_run, spm_realign_wrapper_skeleton
...
def _create_project(client: TestClient, tmp_path: Path) -> dict:
    resp = client.post("/api/projects/create", json={
        "project_name": "Skeleton Project", "rawdata_dir": str(Path("examples/synthetic_bids/rawdata").resolve()),
...
def test_safety_flags_all_true(tmp_path, monkeypatch):
    ...
    for key in ("preview_only", "no_matlab_called", "no_spm_called",
                "no_files_created", "rawdata_not_modified",
                "not_safe_allowlisted", "execution_disabled"):
...
def test_wrapper_response_manifest_items_have_expected_kinds(tmp_path, monkeypatch):
    ...
    if manifests:
        ...
        expected = {"realigned_bold", "mean_bold", "motion_params",
...
def test_wrapper_response_manifest_summary_counts_are_stable(tmp_path, monkeypatch):
    ...
    for key in ("manifest_count", "total_items", "missing_required_count",
                "verified_count", "would_overwrite_count"):
...
def test_wrapper_preview_manifests_are_not_verified(tmp_path, monkeypatch):
    ...
    for m in body["output_manifests"]:
        for item in m.get("items", []):
            assert item["verified"] is False, f"Item {item['kind']} should not be verified in preview"
