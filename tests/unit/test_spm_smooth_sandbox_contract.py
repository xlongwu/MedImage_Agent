...
def _fake_subprocess_run(monkeypatch, *, returncode=0, create_outputs=True):
    def fake_run(cmd, stdout=None, stderr=None, **kw):
        ...
        if returncode == 0:
            ...
            if create_outputs: sf.write_text("fake")
...
def test_normalized_func_passes(monkeypatch, tmp_path):
    _fake_subprocess_run(monkeypatch); _make_env(tmp_path)
...
def test_fwhm_wrong_length_blocked(tmp_path):
    ...
    assert result["ok"] is False; assert result.get("stage") == "fwhm_preflight"
...
def test_output_in_derivatives(monkeypatch, tmp_path):
    ...
    result = run_spm_smooth_subject(
        matlab_command="matlab", spm_dir=str(tmp_path / "spm12"),
        subject_id="sub-001", derivatives_dir=deriv_dir,
        work_dir=str(tmp_path / "work"), log_dir=str(tmp_path / "logs"), approved=True,
    )
...
def test_fake_matlab_nonzero(monkeypatch, tmp_path):
    _fake_subprocess_run(monkeypatch, returncode=7); _make_env(tmp_path)
    result = run_spm_smooth_subject(
        matlab_command="matlab", spm_dir=str(tmp_path / "spm12"),
        subject_id="sub-001", derivatives_dir=str(tmp_path / "derivatives"),
        work_dir=str(tmp_path / "work"), log_dir=str(tmp_path / "logs"), approved=True,
    )
    assert result["ok"] is False; assert result.get("returncode") == 7
