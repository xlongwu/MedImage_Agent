...
from src.backend.app.services.preprocessing_stage_outputs import register_fc_outputs  # noqa: E402
...
def test_registers_metadata_warning(tmp_path, monkeypatch):
    ...
    result = register_fc_outputs(
        "brain-tumor-study",
        "pp-test",
        StageOutputRegistrationRequest(execution_id="fc-ex-abc", confirm_sandbox_outputs=True),
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post(
        "/api/projects/brain-tumor-study/preprocessing/runs/pp-test/stage-outputs/register-fc",
