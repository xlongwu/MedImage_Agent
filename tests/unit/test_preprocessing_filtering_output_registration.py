...
def test_registers_metadata_only_as_warning(tmp_path, monkeypatch):
    ...
    result = register_filtering_outputs("brain-tumor-study", "pp-test",
        StageOutputRegistrationRequest(execution_id="tf-ex-abc", confirm_sandbox_outputs=True), project_dir=str(tmp_path))
    assert result.status == "warning"

def test_blocks_missing_exec(tmp_path):
    result = register_filtering_outputs("test", "pp-test", StageOutputRegistrationRequest(execution_id=""))
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post("/api/projects/brain-tumor-study/preprocessing/runs/pp-test/stage-outputs/register-filtering",
