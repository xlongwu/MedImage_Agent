...
def _isolated_store(tmp_path: Path, monkeypatch) -> SQLiteDesktopStore:
    ...
    for module in (project_routes, dashboard_routes, project_context, reviewed_plan_store, project_history_routes, execute_reviewed_routes, bold_reference_readiness, motion_qc_readiness, spm_realign_dry_run, spm_realign_wrapper_skeleton, nifti_thumbnail):
...
def _create(client: TestClient, tmp_path: Path, rawdata: Path) -> dict:
    ...
    resp = client.post("/api/projects/create", json={
        "project_name": "ThumbTest", "rawdata_dir": str(rawdata),
...
def test_synthetic_3d_returns_three_thumbnails(tmp_path, monkeypatch):
    ...
    client = TestClient(app)
    c = _create(client, tmp_path, rawdata)
    body = client.get(f"/api/projects/{c['project_id']}/nifti-qc/images/nifti_0000/thumbnail?view=all").json()
...
def test_thumbnail_png_base64_decodable(tmp_path, monkeypatch):
    ...
    client = TestClient(app)
    c = _create(client, tmp_path, rawdata)
    body = client.get(f"/api/projects/{c['project_id']}/nifti-qc/images/nifti_0000/thumbnail?view=axial").json()
    if body["thumbnails"]:
...
def test_size_capped_at_256(tmp_path, monkeypatch):
    ...
    body = client.get(f"/api/projects/{c['project_id']}/nifti-qc/images/nifti_0000/thumbnail?size=999").json()
    if body["thumbnails"]:
        t = body["thumbnails"][0]
...
def test_safety_flags_all_true(tmp_path, monkeypatch):
    ...
    client = TestClient(app)
    c = _create(client, tmp_path, rawdata)
    body = client.get(f"/api/projects/{c['project_id']}/nifti-qc/images/nifti_0000/thumbnail").json()
    flags = body["safety_flags"]
    for key in ("read_only", "rawdata_not_modified", "no_preprocessing_executed", "thumbnail_only", "clinical_use_prohibited"):
...
def test_4d_nifti_selected_volume_thumbnail(tmp_path, monkeypatch):
    ...
    client = TestClient(app)
    c = _create(client, tmp_path, rawdata)
    resp = client.get(f"/api/projects/{c['project_id']}/nifti-qc/images/nifti_0000/thumbnail?view=axial&volume_index=1")
...
def test_out_of_range_volume_index_returns_400(tmp_path, monkeypatch):
    ...
    client = TestClient(app)
    c = _create(client, tmp_path, rawdata)
    resp = client.get(f"/api/projects/{c['project_id']}/nifti-qc/images/nifti_0000/thumbnail?volume_index=99")
...
def test_thumbnail_endpoint_ignores_arbitrary_path_query(tmp_path, monkeypatch):
    ...
    client = TestClient(app)
    c = _create(client, tmp_path, rawdata)
    resp = client.get(f"/api/projects/{c['project_id']}/nifti-qc/images/nifti_0000/thumbnail?path=../../secret&view=all")
    assert resp.status_code == 200
