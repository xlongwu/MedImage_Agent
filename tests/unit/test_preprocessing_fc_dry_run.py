...
def test_succeeds_with_filtered(tmp_path, monkeypatch):
    ...
    result = run_fc_dry_run("brain-tumor-study", "pp-test",
        FcDryRunRequest(functional_input_dir=str(func_dir), confirm_dry_run_only=True), project_dir=str(tmp_path))
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post("/api/projects/brain-tumor-study/preprocessing/runs/pp-test/fc/dry-run",
