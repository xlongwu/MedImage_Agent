...
def test_report_exporter_writes_metadata_only_group_summary_when_missing(tmp_path: Path):
    ...
    summary = json.loads((pkg/"summary"/"group_summary"/"dataset_summary.json").read_text(encoding="utf-8"))
    assert summary["ok"] is False
    assert summary["capability_level"] == "metadata_only"
    assert (pkg/"tables"/"subject_metrics_table.csv").exists()

    validation = validate_rsfmri_report_package(exports_dir=str(e), export_id="missing_group_summary")
    assert validation["ok"] is True
    assert validation["stats"]["checksum_mismatch_total"] == 0
    assert validation["stats"]["missing_files_total"] == 0
...
def test_report_exporter_uses_native_preproc_group_summary_bridge(tmp_path: Path):
    d = tmp_path/"derivatives"; rpt = tmp_path/"reports"; w = tmp_path/"work"; e = tmp_path/"exports"
    run = tmp_path/"outputs"/"projects"/"demo"/"preprocessing_native_runs"/"run-native-001"
    group_dir = run/"artifacts"/"group_summary"; group_dir.mkdir(parents=True)
    validation_dir = run/"artifacts"/"validation_report"; validation_dir.mkdir(parents=True)
    final_dir = run/"artifacts"/"final_report"; final_dir.mkdir(parents=True)
        ...
        json.dumps(
            {
                ...
                "data": {
                    "stage_results": [
                        {"stage_id": "functional_connectivity", "status": "succeeded", "warnings": [], "errors": []}
                    ],
                    "validation_summary": {
                        "summary": {
                            "artifact_failed_count": 0,
    ...
    validation = validate_rsfmri_report_package(exports_dir=str(e), export_id="native_bridge_export")
    assert validation["ok"] is True
...
def test_report_exporter_collects_artifacts_from_batch_subject_validations(tmp_path: Path):
    ...
    (group_dir / "native_group_summary.json").write_text(
        json.dumps({
            ...
            "subject_summaries": [{"subject_id": sid, "status": "succeeded"} for sid in subjects],
            "limitations": [],
        }),
        encoding="utf-8",
    )
    ...
    for sid in subjects:
        ...
        (run / sid / "native_full_run_manifest.json").write_text(
            json.dumps({
                "status": "succeeded",
                "stage_results": [
                    {"stage_id": "slice_timing", "status": "succeeded", "warnings": [], "errors": []},
                    {"stage_id": "realignment", "status": "simplified", "warnings": ["translation_only"], "errors": []},
                    {"stage_id": "dicom_to_nifti", "status": "skipped", "warnings": [], "errors": []},
                ],
            }),
            encoding="utf-8",
        )
        ...
        child_validation = artifact_dir / "validation_report" / "native_preproc_validation_report.json"
...
def test_report_export_listing_tolerates_incomplete_stale_export(tmp_path: Path):
    ...
    sub = "sub-001"
    qd = d/"rsfmri_qc"/sub; qd.mkdir(parents=True)
    (qd/"motion_qc.json").write_text(json.dumps({"ok": True, "subject_id": sub}), encoding="utf-8")
