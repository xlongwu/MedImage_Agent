...
SUCCESS_FIELDS = [
    ...
    "guard_status",
    "audit_id",
    "provider_call_allowed_by_adapter",
...
def test_dry_run_no_provider(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(MockGuiProvider, "perform_step", lambda s, se, a, p: calls.append(1))
    _mock_step(dry=True)
    assert len(calls) == 0
...
@pytest.mark.parametrize(
    "fid",
    ["click_run", "provider_pywinauto", "take_screenshot", "read_clipboard", "enter_password"],
)
...
def test_no_sensitive_in_response(fid, mode):
    ...
    dry = mode == "dry"
    r = _mock_step(sid=sid, fid=fid, dry=dry, submit=(mode == "submit"))
    _data = r.text
