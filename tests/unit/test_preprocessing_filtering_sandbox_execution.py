...
def _setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.backend.app.services.preprocessing_filtering_execution.mock_store", store)
    return store
...
def _make_func(tmp_path):
    ...
    (sub / "residual_ssub-001_task-rest_bold.nii").write_text("residual"); return so

_ALL = {"MEDIMAGE_ENABLE_REVIEWED_EXECUTION": "1", "MEDIMAGE_ALLOW_SANDBOXED_TEMPORAL_FILTERING": "1"}
...
def test_disabled_without_env(tmp_path, monkeypatch):
    ...
    result = run_filtering_sandbox_execution("test", "pp-test", FilteringSandboxExecutionRequest(dry_run_id="dr"), env={}, project_dir=str(tmp_path))
    assert result.status == "disabled"

def test_missing_dry_run_blocks(tmp_path, monkeypatch):
    _setup(tmp_path, monkeypatch)
    result = run_filtering_sandbox_execution("brain-tumor-study", "pp-test",
        FilteringSandboxExecutionRequest(dry_run_id="nonexistent"), env=_ALL, project_dir=str(tmp_path))
...
def test_sandbox_copies_and_writes(tmp_path, monkeypatch):
    ...
    def mi(name, *a, **kw):
        if name in ("scipy", "scipy.signal"): raise ImportError("mock")
        return ri(name, *a, **kw)
    monkeypatch.setattr(builtins, "__import__", mi)
    req = FilteringSandboxExecutionRequest(dry_run_id="dr-test", functional_input_dir=str(func_dir), confirm_sandbox_copy=True)
    result = run_filtering_sandbox_execution("brain-tumor-study", "pp-test", req, env=_ALL, project_dir=str(tmp_path))
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post("/api/projects/brain-tumor-study/preprocessing/runs/pp-test/temporal-filtering/execute-sandbox",
