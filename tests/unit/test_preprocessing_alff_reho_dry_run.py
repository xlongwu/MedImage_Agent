...
def _setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.backend.app.services.preprocessing_alff_reho_dry_run.mock_store", store
    )
    return store
...
from src.backend.app.schemas.preprocessing_alff_reho_dry_run import (  # noqa: E402
    AlffRehoDryRunRequest,  # noqa: E402
)
from src.backend.app.services.preprocessing_alff_reho_dry_run import (  # noqa: E402
    run_alff_reho_dry_run,  # noqa: E402
)


def test_blocks_no_func(tmp_path):
    result = run_alff_reho_dry_run(
        "test", "pp-test", AlffRehoDryRunRequest(), project_dir=str(tmp_path)
    )
...
def test_succeeds(tmp_path, monkeypatch):
    ...
    result = run_alff_reho_dry_run(
        "brain-tumor-study",
        "pp-test",
        AlffRehoDryRunRequest(functional_input_dir=str(func_dir), confirm_dry_run_only=True),
        project_dir=str(tmp_path),
    )
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post(
        "/api/projects/brain-tumor-study/preprocessing/runs/pp-test/alff-reho/dry-run",
