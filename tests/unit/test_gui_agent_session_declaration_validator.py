...
def test_rawdata_file_scope_blocked():
    result = validate_gui_session_declaration(**_valid_session(file_scope=["rawdata/sub-001/"]))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_FILE_SCOPE_BLOCKED"


def test_data_file_scope_blocked():
    result = validate_gui_session_declaration(**_valid_session(file_scope=["data/sub-001/"]))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_FILE_SCOPE_BLOCKED"


def test_derivatives_write_scope_blocked():
    result = validate_gui_session_declaration(**_valid_session(file_scope=["derivatives/output/"]))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_FILE_SCOPE_BLOCKED"


def test_traversal_file_scope_blocked():
    result = validate_gui_session_declaration(**_valid_session(file_scope=["../etc/passwd"]))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_FILE_SCOPE_BLOCKED"
...
def test_allow_rawdata_access_true_blocked():
    result = validate_gui_session_declaration(**_valid_session(allow_rawdata_access=True))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_FILE_SCOPE_BLOCKED"


def test_allow_derivatives_write_true_blocked():
    result = validate_gui_session_declaration(**_valid_session(allow_derivatives_write=True))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_FILE_SCOPE_BLOCKED"


def test_human_present_false_blocked():
    result = validate_gui_session_declaration(**_valid_session(human_present=False))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_HUMAN_REQUIRED"


def test_emergency_abort_false_blocked():
    result = validate_gui_session_declaration(**_valid_session(emergency_abort_enabled=False))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_EMERGENCY_ABORT_REQUIRED"


def test_audit_log_required_false_blocked():
    result = validate_gui_session_declaration(**_valid_session(audit_log_required=False))
    assert result.ok is False
...
def test_screenshot_disabled_passes():
    result = validate_gui_session_declaration(**_valid_session(screenshot_policy="disabled"))
    assert result.ok is True


def test_screenshot_ephemeral_blocked_in_t003():
    result = validate_gui_session_declaration(**_valid_session(screenshot_policy="ephemeral_only"))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_SCREENSHOT_BLOCKED"


def test_screenshot_persist_raw_blocked():
    result = validate_gui_session_declaration(**_valid_session(screenshot_policy="persist_raw"))
    assert result.ok is False


def test_clipboard_read_blocked():
    result = validate_gui_session_declaration(**_valid_session(clipboard_policy="read"))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_CLIPBOARD_BLOCKED"


def test_clipboard_read_write_blocked():
    result = validate_gui_session_declaration(**_valid_session(clipboard_policy="read_write"))
    assert result.ok is False


def test_network_local_only_blocked_in_t003():
    result = validate_gui_session_declaration(**_valid_session(network_policy="local_only"))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_NETWORK_BLOCKED"


def test_network_unrestricted_blocked():
    result = validate_gui_session_declaration(**_valid_session(network_policy="unrestricted"))
    assert result.ok is False


def test_external_app_any_blocked():
    result = validate_gui_session_declaration(**_valid_session(external_app_policy="any_app"))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_EXTERNAL_APP_BLOCKED"


def test_redaction_policy_invalid_blocked():
    result = validate_gui_session_declaration(**_valid_session(redaction_policy="none"))
    assert result.ok is False
...
def test_duration_300_passes():
    result = validate_gui_session_declaration(**_valid_session(duration_limit_seconds=300))
    assert result.ok is True


def test_duration_0_blocked():
    result = validate_gui_session_declaration(**_valid_session(duration_limit_seconds=0))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_SESSION_INVALID"


def test_duration_exceeds_max_blocked():
    result = validate_gui_session_declaration(**_valid_session(duration_limit_seconds=301))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_SESSION_INVALID"
