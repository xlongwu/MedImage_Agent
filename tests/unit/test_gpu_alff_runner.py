...
def test_approved_false_blocks():
    r = run_gpu_alff_subject(subject_id="s", input_functional="/d/f.nii",
        derivatives_dir="/d", run_id="r1", tr=2.0, approved=False)
    assert r["ok"] is False


def test_invalid_subject_blocked():
    r = run_gpu_alff_subject(subject_id="", input_functional="/d/f.nii",
        derivatives_dir="/d", run_id="r1", tr=2.0)
    assert r["ok"] is False
...
def test_scoped_derivative_passes(tmp_path):
    ...
    r = run_gpu_alff_subject(subject_id="sub-001", input_functional=func,
        derivatives_dir=deriv, run_id="r1", tr=2.0)
    assert r["ok"] is True
...
def test_rawdata_input_blocked(tmp_path):
    ...
    r = run_gpu_alff_subject(subject_id="s", input_functional=str(raw / "func.nii"),
...
def test_arbitrary_input_blocked(tmp_path):
    _, deriv = _make_derivative(tmp_path)
    r = run_gpu_alff_subject(subject_id="s", input_functional="/etc/func.nii",
        derivatives_dir=deriv, run_id="r", tr=2.0)
    assert r["ok"] is False
...
def test_tr_too_large_blocked(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    r = run_gpu_alff_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", tr=20.0)
    assert r["ok"] is False
...
def test_invalid_device_blocked(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    r = run_gpu_alff_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", tr=2.0, device="cuda:1")
    assert r["ok"] is False
...
def test_timeout_too_large_blocked(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    r = run_gpu_alff_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", tr=2.0, timeout_seconds=200)
    assert r["ok"] is False
