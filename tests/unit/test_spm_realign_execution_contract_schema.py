...
def test_request_defaults_to_disabled():
    req = SpmRealignExecutionRequest(
        project_id="p1", reviewed_plan_id="r1",
    )
...
def test_request_rejects_unknown_fields():
    ...
    with pytest.raises(ValidationError):
        SpmRealignExecutionRequest(
            project_id="p1", reviewed_plan_id="r1",
            shell_command="matlab -batch evil",  # noqa: not a real field
        )
...
def test_request_rejects_matlab_script_field():
    with pytest.raises(ValidationError):
        SpmRealignExecutionRequest(
            project_id="p1", reviewed_plan_id="r1",
            matlab_script="evil()",  # noqa: not a real field
        )
...
def test_output_manifest_counts_missing():
    manifest = SpmRealignOutputManifest(
        project_id="p1", run_id="r1", output_root="/out",
        items=[
            SpmRealignOutputManifestItem(kind="realigned_bold", path="/out/a.nii", required=True, verified=True),
            SpmRealignOutputManifestItem(kind="mean_bold", path="/out/b.nii", required=True, verified=False),
            SpmRealignOutputManifestItem(kind="motion_params", path="/out/c.txt", required=True, verified=False),
        ],
        missing_required_count=2, verified_count=1,
    )
...
def test_provenance_includes_all_fields():
    p = SpmRealignExecutionProvenance(
        project_id="p1", reviewed_plan_id="r1", run_id="run1",
        command_template_id="spm12_realign_estwrite_v1",
        matlab_version="R2023b", spm_version="12.7771",
...
def test_failure_record_all_stages():
    stages = ["preflight", "approval", "audit", "environment",
              "batch_generation", "execution", "output_verification",
              "provenance", "artifact_discovery"]
    for stage in stages:
        f = SpmRealignFailureRecord(
            code="ERR", message="fail", stage=stage,
            retryable=True, next_action="fix it",
        )
...
def test_result_defaults_executor_called_false():
    result = SpmRealignExecutionResult(
        ok=False, project_id="p1", reviewed_plan_id="r1",
        run_id="run1", status="not_started",
    )
...
def test_result_can_represent_blocked():
    result = SpmRealignExecutionResult(
        ok=False, project_id="p1", reviewed_plan_id="r1",
        run_id="run1", status="blocked",
        failures=[
            SpmRealignFailureRecord(
                code="APPROVAL_REQUIRED", message="Approval missing",
                stage="approval", retryable=False,
...
def test_result_can_represent_failed_output_verification():
    result = SpmRealignExecutionResult(
        ...
        output_manifests=[
            SpmRealignOutputManifest(
                project_id="p1", run_id="run1", output_root="/out",
                items=[
                    SpmRealignOutputManifestItem(kind="realigned_bold", path="/out/a.nii", required=True, verified=False),
                ],
        ...
        failures=[
            SpmRealignFailureRecord(
                code="OUTPUT_MISSING", message="Expected output not found",
...
def test_request_serializes_to_json_dict():
    req = SpmRealignExecutionRequest(
        project_id="p1", reviewed_plan_id="r1",
        subject_scope=["sub-01"], params={"quality": 0.9},
    )
    d = req.model_dump()
...
def test_no_files_created(tmp_path):
    ...
    SpmRealignExecutionResult(ok=False, project_id="p2", reviewed_plan_id="r2", run_id="r2", status="not_started")
