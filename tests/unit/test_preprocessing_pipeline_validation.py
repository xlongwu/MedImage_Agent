...
def _setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.backend.app.services.preprocessing_pipeline_validation.mock_store", store
    )
    return store


from src.backend.app.services.preprocessing_pipeline_validation import (  # noqa: E402
...
def test_not_started_for_missing_run(tmp_path, monkeypatch):
    ...
    result = validate_preprocessing_pipeline(
        "brain-tumor-study", "pp-test", project_dir=str(tmp_path)
    )
...
def test_detects_partial_pipeline(tmp_path, monkeypatch):
    _setup(tmp_path, monkeypatch)
    rd = tmp_path / "preprocessing_runs" / "pp-test"
...
def test_validation_reloads_metric_maps_and_atlas(tmp_path, monkeypatch):
        ...
        "artifacts": [
            {
                "artifact_id": "a1",
                "stage_id": "alff_falff",
                "artifact_type": "alff_map",
                "path": str(alff),
                "path_kind": "local_runtime",
            },
            {
                "artifact_id": "a2",
                "stage_id": "alff_falff",
                "artifact_type": "falff_map",
                "path": str(falff),
                "path_kind": "local_runtime",
            },
            {
    ...
    (run_dir / "preprocessing_artifact_registry.json").write_text(
