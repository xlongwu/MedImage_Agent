...
def test_smoke_generates_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.backend.app.services.spm_runtime._gen_synthetic_nifti",
        lambda d: (
            d / "synth_bold.nii" if (d / "synth_bold.nii").write_text("fake") or True else None
        ),
    )
    from src.backend.app.schemas.spm_runtime import SpmSyntheticSmokeRequest
    from src.backend.app.services.spm_runtime import run_synthetic_spm_smoke
    ...
    result = run_synthetic_spm_smoke(
        "brain-tumor-study", req, env=_ALL_FLAGS, project_dir=str(tmp_path)
    )
    assert result.ok
