...
def test_python_alff_falff_outputs_metric_maps(tmp_path: Path):
    ...
    sid = "sub-001"
    fd = derivatives / "rsfmri_preproc" / sid / "func"
    qd = derivatives / "rsfmri_qc" / sid
    fd.mkdir(parents=True)
    qd.mkdir(parents=True)
    rp = fd / "resid_swrasub-001_bold.nii"
    fp = fd / "filt_resid_swrasub-001_bold.nii"
    tr = 2.0
    nt = 32
    t = np.arange(nt) * tr
    rs = (np.sin(2 * np.pi * 0.03 * t) + 0.5 * np.sin(2 * np.pi * 0.2 * t)).astype(np.float32)
    fs = np.sin(2 * np.pi * 0.03 * t).astype(np.float32)
    rd = np.zeros((3, 3, 3, nt), dtype=np.float32)
    rd[:] = rs
    fd_data = np.zeros((3, 3, 3, nt), dtype=np.float32)
    fd_data[:] = fs
    ...
    qp = qd / "alff_falff_qc.json"
    assert qp.exists()
    pl = json.loads(qp.read_text(encoding="utf-8"))
    assert pl["subject_id"] == sid
    assert pl["alff_qc_status"] in {"PASS", "WARNING"}
    assert pl["retained_frequency_bin_count"] > 0
    assert pl["falff_mean"] >= 0
