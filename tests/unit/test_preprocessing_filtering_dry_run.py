...
def _setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.backend.app.services.preprocessing_filtering_dry_run.mock_store", store
    )
    return store
...
def _make_func(tmp_path):
    ...
    (sub / "res_sub-001_task-rest_bold.nii").write_text("residual")
    return so


from src.backend.app.schemas.preprocessing_filtering_dry_run import (  # noqa: E402
    FilteringDryRunRequest,  # noqa: E402
)
from src.backend.app.services.preprocessing_filtering_dry_run import (  # noqa: E402
    run_filtering_dry_run,  # noqa: E402
)


def test_blocks_no_func(tmp_path):
    result = run_filtering_dry_run(
        "test", "pp-test", FilteringDryRunRequest(), project_dir=str(tmp_path)
    )
...
def test_blocks_invalid_cutoff(tmp_path, monkeypatch):
    ...
    result = run_filtering_dry_run(
        ...
        FilteringDryRunRequest(
            functional_input_dir=str(func_dir), low_cut_hz=0.09, high_cut_hz=0.08
        ),
...
def test_writes_filter_design(tmp_path, monkeypatch):
    ...
    result = run_filtering_dry_run(
        "brain-tumor-study",
        "pp-test",
        FilteringDryRunRequest(functional_input_dir=str(func_dir), confirm_dry_run_only=True),
        project_dir=str(tmp_path),
    )
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post(
        "/api/projects/brain-tumor-study/preprocessing/runs/pp-test/temporal-filtering/dry-run",
