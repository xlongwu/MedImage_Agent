...
def test_endpoint_is_read_only():
    ...
    resp = client.post(
        "/api/projects/brain-tumor-study/conversion/release-readiness/conv-test", json={}
    )
...
def test_endpoint_does_not_call_dcm2niix():
    ...
    code_lines = [
        line for line in source.splitlines() if '"""' not in line and "dcm2niix" not in line.lower()
    ]
    code = "\n".join(code_lines)
...
def test_endpoint_returns_safety_flags():
    ...
    assert sf["public_endpoint_disabled"] is (sf["public_endpoint_state"] == "absent")
