...
def test_dry_run_succeeds_with_bold_files(tmp_path, monkeypatch):
    _setup_store(tmp_path, monkeypatch); cb = _make_bold_input(tmp_path)
    ...
    assert result.ok; assert result.bold_count == 2; assert len(result.batch_preview_paths) == 2


def test_dry_run_blocks_zero_bold(tmp_path, monkeypatch):
    _setup_store(tmp_path, monkeypatch); empty = tmp_path / "empty"; empty.mkdir()
...
def test_dry_run_warns_missing_tr(tmp_path, monkeypatch):
    _setup_store(tmp_path, monkeypatch); cb = _make_bold_input(tmp_path)
    for jf in cb.rglob("*.json"): jf.unlink()
    req = SliceTimingRealignDryRunRequest(preprocessing_input_dir=str(cb), confirm_dry_run_only=True)
    result = run_slice_timing_realign_dry_run("brain-tumor-study", "pp-test", req, project_dir=str(tmp_path))
...
def test_dry_run_writes_manifest(tmp_path, monkeypatch):
    ...
    assert mf.exists(); assert json.loads(mf.read_text())["status"] == "dry_run_preview"
...
def test_dry_run_endpoint_returns_200(tmp_path):
    ...
    resp = client.post("/api/projects/brain-tumor-study/preprocessing/runs/pp-test/spm/slice-timing-realign/dry-run",
