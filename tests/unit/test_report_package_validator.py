...
def test_validator_passes_valid_package(tmp_path: Path):
    e = tmp_path / "exports"
    root = e / "rsfmri_report_package"
    pkg = root / "test_export"
    pkg.mkdir(parents=True)
    rm = pkg / "README.md"
    ix = pkg / "index.md"
    es = pkg / "export_summary.json"
    rm.write_text("# README\n", encoding="utf-8")
    ix.write_text("# Index\n", encoding="utf-8")
    es.write_text(json.dumps({"ok": True}), encoding="utf-8")
    safety = {
    ...
    files = [
        {
            "relative_path": str(f.relative_to(pkg)),
            "source_path": None,
...
def test_validator_latest_skips_incomplete_stale_exports(tmp_path: Path):
    ...
    stale = root / "rsfmri_export_20990101_000000"
    stale.mkdir()
    valid = root / "rsfmri_export_20260706_220533"
    valid.mkdir()
    rm = valid / "README.md"
    ix = valid / "index.md"
    es = valid / "export_summary.json"
    rm.write_text("# README\n", encoding="utf-8")
    ix.write_text("# Index\n", encoding="utf-8")
    es.write_text(json.dumps({"ok": True}), encoding="utf-8")
    safety = {
    ...
    files = [
        {
            "relative_path": f.relative_to(valid).as_posix(),
            "source_path": None,
            "size_bytes": f.stat().st_size,
            "sha256": _sh(f),
            "category": "test",
    ...
    csp = valid / "checksums" / "SHA256SUMS.txt"
    csp.parent.mkdir(parents=True)
    csp.write_text(
