...
def _isolated_store(tmp_path: Path, monkeypatch) -> SQLiteDesktopStore:
    ...
    for module in (project_routes, dashboard_routes, project_context, reviewed_plan_store, project_history_routes, execute_reviewed_routes, bold_reference_readiness, motion_qc_readiness,
        mock_store_module,
    ):
...
def _create_project(client: TestClient, tmp_path: Path) -> dict:
    resp = client.post("/api/projects/create", json={
        "project_name": "DryRun Project", "rawdata_dir": str(Path("examples/synthetic_bids/rawdata").resolve()),
...
def test_safety_flags_all_true(tmp_path, monkeypatch):
    ...
    for key in ("dry_run_only", "rawdata_not_modified", "no_files_created",
                "no_matlab_called", "no_spm_called", "execution_disabled",
