...
from src.backend.app.schemas.preprocessing_nuisance_dry_run import (  # noqa: E402
    NuisanceDryRunRequest,  # noqa: E402
)
from src.backend.app.services.preprocessing_nuisance_dry_run import (  # noqa: E402
    run_nuisance_dry_run,  # noqa: E402
)


def test_blocks_no_func(tmp_path):
    result = run_nuisance_dry_run(
        "test", "pp-test", NuisanceDryRunRequest(), project_dir=str(tmp_path)
    )
...
def test_succeeds_with_smoothed(tmp_path, monkeypatch):
    ...
    func_dir = _make_smoothed(tmp_path)
    result = run_nuisance_dry_run(
        "brain-tumor-study",
        "pp-test",
        NuisanceDryRunRequest(functional_input_dir=str(func_dir), confirm_dry_run_only=True),
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post(
        "/api/projects/brain-tumor-study/preprocessing/runs/pp-test/nuisance-regression/dry-run",
