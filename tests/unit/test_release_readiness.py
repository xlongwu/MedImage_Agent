...
def test_result_has_expected_keys(result):
    for key in ["ok", "release_readiness_status", "checks_total", "checks_pass",
                "checks_fail", "checks", "warnings", "errors", "outputs"]:
...
def test_m1_docs_all_present(checks):
    for name in M1_DOC_PATHS:
        key = ("docs_m1", name)
        assert key in checks, f"Missing M1 doc check: {name}"
        assert checks[key] == "PASS", f"M1 doc {name} is {checks[key]}"


def test_m1_docs_count(checks):
    m1_keys = [k for k in checks if k[0] == "docs_m1" and k[1].startswith("file:")]
    assert len(m1_keys) >= len(M1_DOC_PATHS)


# ── AGENTS.md content checks ──

def test_agents_md_contains_rawdata(checks):
    assert checks.get(("docs_m1", "AGENTS.md contains 'rawdata'")) == "PASS"


def test_agents_md_contains_approval_gate(checks):
    assert checks.get(("docs_m1", "AGENTS.md contains 'approval gate'")) == "PASS"


def test_agents_md_contains_forbidden(checks):
    assert checks.get(("docs_m1", "AGENTS.md contains '禁止'")) == "PASS"


def test_agents_md_contains_llm(checks):
    assert checks.get(("docs_m1", "AGENTS.md contains 'LLM'")) == "PASS"
...
def test_readme_uses_correct_backend_command(checks):
    assert checks[("docs_m1", "README.md uses src.backend.app.main:app")] == "PASS"


def test_readme_uses_correct_frontend_command(checks):
    assert checks[("docs_m1", "README.md uses cd src/frontend")] == "PASS"
...
def test_import_diagnostics_frontend_panel(checks):
    assert checks[("frontend", "import diagnostics panel")] == "PASS"
...
def test_output_files_written(result):
    ...
    for fname in ["release_readiness_result.json", "release_readiness_report.md",
                  "release_readiness_checklist.csv", "release_readiness_dashboard.json"]:
        fpath = out_dir / fname
