...
def test_register_converted_bids_counts_correctly(tmp_path, monkeypatch):
    ...
    assert result.ok
    assert result.bold_count == 3
    assert result.t1w_count == 3
...
def test_register_fails_when_dir_missing(tmp_path, monkeypatch):
    ...
    req = PreprocessingInputRegistrationRequest(
        conversion_run_id="conv-test", converted_bids_dir=str(tmp_path / "nonexistent")
    )
    result = register_converted_bids_as_preprocessing_input("brain-tumor-study", req)
    assert result.status == "blocked"
...
def test_register_blocks_path_traversal(tmp_path, monkeypatch):
    ...
    req = PreprocessingInputRegistrationRequest(
        conversion_run_id="conv-test", converted_bids_dir="../outside/converted"
    )
    result = register_converted_bids_as_preprocessing_input("brain-tumor-study", req)
    assert result.status == "blocked"
...
def test_register_reports_missing_t1w(tmp_path, monkeypatch):
    ...
    req = PreprocessingInputRegistrationRequest(
        conversion_run_id="conv-test", converted_bids_dir=str(cb)
    )
    result = register_converted_bids_as_preprocessing_input("brain-tumor-study", req)
...
def test_register_writes_metadata(tmp_path, monkeypatch):
    _store = _setup_store(tmp_path, monkeypatch)
...
def test_register_endpoint_returns_200(tmp_path):
    ...
    resp = client.post(
        "/api/projects/brain-tumor-study/preprocessing/input/register-converted",
        json={"conversion_run_id": "conv-test", "converted_bids_dir": str(cb)},
    )
...
def test_register_endpoint_404_for_missing_project():
    ...
    client = TestClient(app)
    resp = client.post(
        "/api/projects/nonexistent/preprocessing/input/register-converted",
        json={"conversion_run_id": "conv-test"},
    )
