...
def test_every_fixture_has_expected_status():
    for f in FIXTURES:
        assert f.expected_status in (
            "NORMALIZED_ACTION_READY", "MODEL_ACTION_REJECTED"
...
def test_every_model_output_has_timestamp():
    for f in FIXTURES:
        assert f.model_output.get("timestamp"), (
            f"Fixture {f.fixture_id} missing timestamp"
        )
...
SAFE_IDS = ["safe_observe_current_state", "safe_record_observation",
            "safe_note_visible_state"]
...
REJECTION_FIXTURES = [f for f in FIXTURES
                      if f.expected_status == "MODEL_ACTION_REJECTED"]
...
def test_rejection_fixture_expected_reason_matches(monkeypatch=None):
    ...
    r = validate_and_normalize_model_output(
        **get_mock_model_fixture("disable_guard").model_output)
    ...
    r = validate_and_normalize_model_output(
        **get_mock_model_fixture("read_clipboard").model_output)
    ...
    r = validate_and_normalize_model_output(
        **get_mock_model_fixture("upload_file").model_output)
    assert r.adapter_rejection_reason == "network_request_blocked"

    r = validate_and_normalize_model_output(
        **get_mock_model_fixture("enter_password").model_output)
    assert r.adapter_rejection_reason == "credential_request_blocked"

    r = validate_and_normalize_model_output(
        **get_mock_model_fixture("open_browser").model_output)
    assert r.adapter_rejection_reason == "network_request_blocked"
...
CATEGORIES_REQUIRED = {
    "safe_observation", "ambiguous", "unknown",
    "raw_coordinate", "tier_1", "tier_2", "tier_3",
    "keyboard_mouse", "provider_bypass", "approval_bypass",
    "policy_override", "screenshot", "clipboard",
    "file_path", "rawdata", "derivatives", "network",
    "credentials", "shell", "injection", "structural",
}
