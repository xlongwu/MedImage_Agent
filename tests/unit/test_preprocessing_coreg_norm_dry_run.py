...
def _setup_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.backend.app.services.preprocessing_coreg_norm_dry_run.mock_store", store)
    return store
...
def _make_t1w(tmp_path):
    cb = tmp_path / "cb"; sub = cb / "sub-001" / "anat"; sub.mkdir(parents=True)
    (sub / "sub-001_T1w.nii.gz").write_text("t1w"); return cb
...
def test_blocks_no_func_input(tmp_path, monkeypatch):
    ...
    result = run_coreg_norm_dry_run("brain-tumor-study", "pp-test", CoregNormDryRunRequest(), project_dir=str(tmp_path))
    assert result.status == "blocked"


def test_dry_run_succeeds(tmp_path, monkeypatch):
    store = _setup_store(tmp_path, monkeypatch)
    sandbox = _make_func_input(tmp_path); t1w_dir = _make_t1w(tmp_path)
    proj = store.get_project("brain-tumor-study")
    if not isinstance(proj.metadata, dict): proj.metadata = {}
    proj.metadata["current_functional_input_dir"] = str(sandbox)
    proj.metadata["preprocessing_input_dir"] = str(t1w_dir)
    result = run_coreg_norm_dry_run("brain-tumor-study", "pp-test",
        CoregNormDryRunRequest(functional_input_dir=str(sandbox), confirm_dry_run_only=True), project_dir=str(tmp_path))
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post("/api/projects/brain-tumor-study/preprocessing/runs/pp-test/spm/coreg-normalize/dry-run",
