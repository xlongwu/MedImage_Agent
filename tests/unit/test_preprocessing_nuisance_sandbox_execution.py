...
def _setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.backend.app.services.preprocessing_nuisance_execution.mock_store", store)
    return store
...
def _make_func(tmp_path):
    ...
    (sub / "rp_sub-001.txt").write_text("motion"); return so

_ALL = {"MEDIMAGE_ENABLE_REVIEWED_EXECUTION": "1", "MEDIMAGE_ALLOW_SANDBOXED_NUISANCE_REGRESSION": "1"}
...
def test_disabled_without_env(tmp_path, monkeypatch):
    ...
    result = run_nuisance_sandbox_execution("test", "pp-test", req, env={}, project_dir=str(tmp_path))
    assert result.status == "disabled"

def test_missing_dry_run_blocks(tmp_path, monkeypatch):
    _setup(tmp_path, monkeypatch)
    result = run_nuisance_sandbox_execution("brain-tumor-study", "pp-test",
        NuisanceSandboxExecutionRequest(dry_run_id="nonexistent"), env=_ALL, project_dir=str(tmp_path))
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post("/api/projects/brain-tumor-study/preprocessing/runs/pp-test/nuisance-regression/execute-sandbox",
