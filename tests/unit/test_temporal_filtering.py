...
def test_python_temporal_filter_outputs_filtered_nifti(tmp_path: Path):
    ...
    func_dir.mkdir(parents=True); qc_dir.mkdir(parents=True)
    ...
    data = np.zeros((3,3,3,n_time), dtype=np.float32); data[:] = signal
    nib.save(nib.Nifti1Image(data, affine=np.eye(4)), str(input_nii))
    (qc_dir / "slice_timing_qc.json").write_text(json.dumps({"ok": True, "subject_id": subject_id, "tr": tr, "slice_timing_status": "PASS"}), encoding="utf-8")
    result = run_python_temporal_filter_subject(subject_id=subject_id, derivatives_dir=str(derivatives), low_hz=0.01, high_hz=0.08)
    assert result["ok"] is True
    assert (func_dir / "filt_resid_swrasub-001_bold.nii").exists()
    qc_path = qc_dir / "temporal_filtering_qc.json"; assert qc_path.exists()
    ...
    assert payload["tr"] == tr; assert payload["retained_frequency_bin_count"] > 0
...
def test_python_temporal_filter_reads_tr_from_bids_sidecar(tmp_path: Path):
    ...
    result = run_python_temporal_filter_subject(subject_id=subject_id, derivatives_dir=str(derivatives))

    assert result["ok"] is True, result["errors"]
    payload = json.loads((derivatives / "rsfmri_qc" / subject_id / "temporal_filtering_qc.json").read_text(encoding="utf-8"))
...
def test_python_temporal_filter_blocks_missing_tr(tmp_path: Path):
    ...
    nib.save(nib.Nifti1Image(np.zeros((3,3,3,10), dtype=np.float32), affine=np.eye(4)), str(input_nii))
