...
def test_invalid_subject_blocked():
    r = run_gpu_reho_subject(subject_id="", input_functional="/d/f.nii",
        derivatives_dir="/d", run_id="r")
    assert r["ok"] is False
...
def test_scoped_derivative_passes(tmp_path):
    ...
    r = run_gpu_reho_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r")
    assert r["ok"] is True
...
def test_rawdata_blocked(tmp_path):
    ...
    r = run_gpu_reho_subject(subject_id="s", input_functional=str(raw / "f.nii"),
        derivatives_dir=deriv, run_id="r")
    assert r["ok"] is False
...
def test_arbitrary_input_blocked(tmp_path):
    _, deriv = _make_derivative(tmp_path)
    r = run_gpu_reho_subject(subject_id="s", input_functional="/tmp/f.nii",
        derivatives_dir=deriv, run_id="r")
    assert r["ok"] is False
...
def test_valid_mask_passes(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    mask = Path(deriv) / "mask.nii"; mask.write_bytes(b"\x00")
    r = run_gpu_reho_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", mask_path=str(mask))
    assert r["ok"] is True


def test_rawdata_mask_blocked(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    raw = tmp_path / "rawdata"; raw.mkdir(); (raw / "m.nii").write_bytes(b"\x00")
    r = run_gpu_reho_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", mask_path=str(raw / "m.nii"))
    assert r["ok"] is False
...
def test_neighborhood_27_passes(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    r = run_gpu_reho_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", neighborhood=27)
    assert r["ok"] is True
...
def test_neighborhood_0_blocked(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    r = run_gpu_reho_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", neighborhood=0)
    assert r["ok"] is False
...
def test_neighborhood_125_blocked(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    r = run_gpu_reho_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", neighborhood=125)
    assert r["ok"] is False
...
def test_invalid_device_blocked(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    r = run_gpu_reho_subject(subject_id="s", input_functional=func,
        derivatives_dir=deriv, run_id="r", device="cuda:1")
    assert r["ok"] is False
...
def test_output_in_derivatives(tmp_path):
    func, deriv = _make_derivative(tmp_path)
    r = run_gpu_reho_subject(subject_id="sub-001", input_functional=func,
        derivatives_dir=deriv, run_id="r1", dry_run=True)
