...
def _setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.backend.app.services.preprocessing_smoothing_dry_run.mock_store", store
    )
    return store
...
from src.backend.app.schemas.preprocessing_smoothing_dry_run import (  # noqa: E402
    SmoothingDryRunRequest,  # noqa: E402
)
from src.backend.app.services.preprocessing_smoothing_dry_run import (  # noqa: E402
    run_smoothing_dry_run,  # noqa: E402
)


def test_blocks_no_func_input(tmp_path):
    result = run_smoothing_dry_run(
        "test", "pp-test", SmoothingDryRunRequest(), project_dir=str(tmp_path)
    )
...
def test_succeeds_with_func(tmp_path, monkeypatch):
    ...
    sandbox = _make_registered_output(tmp_path)
    result = run_smoothing_dry_run(
        "brain-tumor-study",
        "pp-test",
        SmoothingDryRunRequest(functional_input_dir=str(sandbox), confirm_dry_run_only=True),
        project_dir=str(tmp_path),
    )
    assert result.ok
    assert result.functional_input_count == 1
...
def test_endpoint_200(tmp_path):
    ...
    resp = client.post(
        "/api/projects/brain-tumor-study/preprocessing/runs/pp-test/spm/smoothing/dry-run",
