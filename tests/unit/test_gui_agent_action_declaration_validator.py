...
def test_declared_tier_mismatch_blocked():
    result = validate_gui_action_declaration(**_valid_action(declared_action_tier=1))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_ACTION_TIER_MISMATCH"


def test_declared_tier_missing_blocked():
    result = validate_gui_action_declaration(**_valid_action(declared_action_tier=None))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_ACTION_TIER_MISMATCH"


def test_tier_not_in_session_tiers_blocked():
    result = validate_gui_action_declaration(**_valid_action(session_allowed_action_tiers=[1]))
    assert result.ok is False
...
def test_input_paths_non_empty_blocked():
    result = validate_gui_action_declaration(**_valid_action(input_paths=["some/file.nii"]))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_FILE_SCOPE_BLOCKED"


def test_output_paths_non_empty_blocked():
    result = validate_gui_action_declaration(**_valid_action(output_paths=["some/output.json"]))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_FILE_SCOPE_BLOCKED"


def test_side_effects_writes_file_blocked():
    result = validate_gui_action_declaration(**_valid_action(expected_side_effects="writes_file"))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_SIDE_EFFECT_BLOCKED"


def test_confirmation_required_blocked_in_t004():
    result = validate_gui_action_declaration(**_valid_action(requires_per_action_confirmation=True))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_CONFIRMATION_UNSUPPORTED"


def test_rollback_non_none_blocked():
    result = validate_gui_action_declaration(**_valid_action(rollback_plan="undo_click"))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_ACTION_INVALID"


def test_stop_conditions_missing_blocked():
    result = validate_gui_action_declaration(**_valid_action(stop_conditions=None))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_ACTION_INVALID"


def test_stop_conditions_empty_blocked():
    result = validate_gui_action_declaration(**_valid_action(stop_conditions=[]))
    assert result.ok is False
    assert result.error_code == "GUI_GUARD_ACTION_INVALID"
