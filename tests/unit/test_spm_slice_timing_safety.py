...
def test_compound_command_blocked(tmp_path):
    result = run_spm_slice_timing_subject(
        matlab_command="matlab && rm -rf /", spm_dir=str(tmp_path / "spm12"),
        subject_id="sub-001", input_bold=str(_make_synthetic_bold(tmp_path)),
        derivatives_dir=str(tmp_path / "derivatives"), work_dir=str(tmp_path / "work"),
        log_dir=str(tmp_path / "logs"), approved=True,
    )
...
def test_spm_dir_derivatives_blocked(tmp_path):
    d = tmp_path / "derivatives"; d.mkdir()
    result = run_spm_slice_timing_subject(
        matlab_command="matlab", spm_dir=str(d),
        subject_id="sub-001", input_bold=str(_make_synthetic_bold(tmp_path)),
        derivatives_dir=str(tmp_path / "derivatives"), work_dir=str(tmp_path / "work"),
        log_dir=str(tmp_path / "logs"), approved=True,
    )
...
def test_approved_false_blocks(tmp_path):
    result = run_spm_slice_timing_subject(
        ...
        subject_id="sub-001", input_bold=str(_make_synthetic_bold(tmp_path)),
        derivatives_dir=str(tmp_path / "derivatives"), work_dir=str(tmp_path / "work"),
        log_dir=str(tmp_path / "logs"), approved=False,
    )
    assert result["ok"] is False
...
def test_arbitrary_input_rejected(tmp_path):
    ...
    assert "unsafe" in str(result["errors"]).lower() or "non-synthetic" in str(result["errors"]).lower()
