...
def test_approved_false_blocks():
    r = run_gpu_nuisance_regression_subject(subject_id="s",
        input_functional="/d/f.nii", confounds_path="/d/c.tsv",
...
def test_rawdata_confounds_blocked(tmp_path):
    raw = tmp_path / "rawdata"; raw.mkdir(); (raw / "c.tsv").write_text("a\n1\n")
...
def test_n_confounds_too_large_blocked(tmp_path):
    ...
    r = run_gpu_nuisance_regression_subject(subject_id="s",
        input_functional=func, confounds_path=conf, derivatives_dir=deriv, run_id="r",
        n_confounds=100, timepoints=200)
    assert r["ok"] is False
