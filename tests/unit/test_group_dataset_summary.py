...
def test_group_dataset_summary_aggregates_subject_qc(tmp_path: Path):
    ...
    fcd = d / "rsfmri_fc" / sub
    cd = d / "rsfmri_confounds" / sub
    qd.mkdir(parents=True)
    md.mkdir(parents=True)
    fcd.mkdir(parents=True)
    ...
    (qd / "alff_falff_qc.json").write_text(
        json.dumps(
            {
                "ok": True,
                "subject_id": sub,
    ...
    (qd / "functional_connectivity_qc.json").write_text(
        json.dumps(
            {
                "ok": True,
                "subject_id": sub,
    ...
    (w / "gpu" / "contracts" / "t.json").write_text(
        json.dumps(
            {
                "ok": True,
                "backend_id": "gpu_test",
    ...
    result = build_group_dataset_summary(
        derivatives_dir=str(d), reports_dir=str(rpt), work_dir=str(w)
    )
    ...
    assert (gd / "dataset_summary.json").exists()
    assert (gd / "dashboard_data.json").exists()
    assert (gd / "subject_metrics_table.csv").exists()
    assert (gd / "dataset_summary_report.md").exists()
    db = json.loads((gd / "dashboard_data.json").read_text(encoding="utf-8"))
    assert db["summary_cards"]["subjects_total"] == 1
