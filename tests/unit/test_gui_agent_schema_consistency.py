...
RESPONSE_SCENARIOS = [
    ...
    ("guard_blocked", lambda: _step(fid="safe_observe_current_state", sid="nonexistent_id_xyz")),
    ("fixture_not_found", lambda: _step(fid="no_such")),
    (
        "session_required",
        lambda: client.post(
            "/api/gui-agent/mock-adapter/step", json={"fixture_id": "safe", "submit_to_guard": True}
        ),
    ),
]
...
def test_guard_only_sets_provider_true_on_success():
    ...
    assert (
        rej.json().get("provider_call_allowed_by_guard") is None
    )  # not present in rejected schema
