...
from src.backend.app.native_preproc.stages.alff_falff import compute_alff_falff_maps, run_alff
...
from src.backend.app.native_preproc.stages.nuisance_regression import (
    regress_confounds_with_backend,
    run_nuisance_regression,
)
from src.backend.app.native_preproc.stages.smoothing import smooth_spatial_with_backend
from src.backend.app.native_preproc.stages.temporal_filtering import (
    run_temporal_filtering,
    temporal_filter_4d,
)
...
def test_native_nuisance_regression_cpu_cupy_consistency() -> None:
    ...
    design = np.column_stack([np.ones(24), np.linspace(-1.0, 1.0, 24), rng.normal(size=24)]).astype(
        np.float32
    )
...
def test_native_functional_connectivity_cpu_cupy_consistency_with_constant_roi() -> None:
    ...
    roi = np.column_stack([rng.normal(size=24), rng.normal(size=24), np.ones(24)]).astype(
        np.float32
    )
    cpu = compute_roi_functional_connectivity(roi, roi_names=["a", "b", "constant"])
    gpu = compute_roi_functional_connectivity(
        roi, roi_names=["a", "b", "constant"], compute_policy=GPU_POLICY
    )
    assert gpu[2]["compute"]["actual_backend"] == "gpu-cupy"
    assert cpu[3] == gpu[3]
...
def test_native_gpu_stage_artifacts_are_reloadable_and_provenance_is_truthful(tmp_path) -> None:
    ...
    filtered = run_temporal_filtering(
        input_bold, tmp_path / "filtered", tr=2.0, compute_policy=GPU_POLICY
    )
    ...
    filtered_artifact = next(
        item for item in filtered.output_artifacts if item.artifact_type == "filtered_bold"
    )
...
def test_experimental_reho_gpu_exactly_matches_tie_corrected_cpu_reference(
    neighborhood: int,
) -> None:
    _require_cupy_device()
...
def test_spatial_smoothing_and_label_resampling_match_cpu_contracts() -> None:
    ...
    sigma = (1.1, 0.8, 0.6)
    cpu_smoothed, _ = smooth_spatial_with_backend(
        image, sigma, compute_policy=NativeComputePolicy(backend="cpu")
    )
    gpu_smoothed, gpu_smoothing = smooth_spatial_with_backend(
        image, sigma, compute_policy=GPU_POLICY
    )
