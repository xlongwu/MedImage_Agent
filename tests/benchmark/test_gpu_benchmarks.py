...
class TestGPUBenchmarks:
    ...
    def test_benchmark_reho(self):
        ...
        for size in [(16, 16, 12, 50), (32, 32, 16, 100)]:
            ...
            t0 = time.perf_counter()
            warm_r = compute_reho_cupy(data, neighborhood=27)
            warm_t = time.perf_counter() - t0

            if gpu_r["ok"]:
                assert warm_r["ok"]
                print(
                    f"  ReHo {size}: CPU={cpu_r['runtime_seconds']}s, cold={cold_t:.3f}s, warm={warm_t:.3f}s, warm_speedup={cpu_t / max(warm_t, 0.001):.1f}x"
                )
    ...
    def test_benchmark_nuisance_regression(self):
        ...
        if gpu_r["ok"]:
            assert warm_r["ok"]
            print(
                f"  NuisanceReg: CPU={cpu_r['runtime_seconds']}s, cold={gpu_r['runtime_seconds']}s, warm={warm_r['runtime_seconds']}s"
            )
    ...
    def test_benchmark_temporal_filtering(self):
        ...
        if gpu_r["ok"]:
            assert warm_r["ok"]
            print(
                f"  TemporalFilter: CPU={cpu_r['runtime_seconds']}s, cold={gpu_r['runtime_seconds']}s, warm={warm_r['runtime_seconds']}s"
            )
    ...
    def test_benchmark_functional_connectivity(self):
        ...
        if gpu_r["ok"]:
            assert warm_r["ok"]
            print(
                f"  FC: CPU={cpu_r['runtime_seconds']}s, cold={gpu_r['runtime_seconds']}s, warm={warm_r['runtime_seconds']}s"
            )
