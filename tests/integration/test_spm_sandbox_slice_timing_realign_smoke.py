...
def test_sandbox_smoke_disabled_without_env():
    ...
    required = ["MEDIMAGE_MATLAB_ENABLED", "MEDIMAGE_SPM_SMOKE_ENABLED",
...
def test_sandbox_smoke_end_to_end(tmp_path):
    ...
    req = SpmSandboxExecutionRequest(
        dry_run_id="dr-test", preprocessing_input_dir=test_input,
        confirm_sandbox_copy=True, confirm_no_rawdata_modification=True,
        confirm_slice_timing_realign_only=True, confirm_no_full_preprocessing=True,
        confirm_research_use_only=True, timeout_seconds=300,
    )
    result = run_sandbox_spm_execution("brain-tumor-study", "pp-test", req, project_dir=str(tmp_path))
