...
_ALL_FLAGS = dict.fromkeys(REAL_DCM2NIIX_SYNTHETIC_SMOKE_REQUIRED_FLAGS, "1")
...
@pytest.mark.skipif(
    not _all_flags_present(), reason="Real dcm2niix smoke requires all env flags set to '1'"
)
...
def test_real_smoke_on_synthetic_dicom(tmp_path):
    ...
    if result.status == "succeeded":
        ...
        _outputs = list(output_root.rglob("*.nii*"))
...
def test_refuses_real_rawdata_output_path():
    ...
    result = run_real_dcm2niix_synthetic_smoke(
        input_dir=Path("/tmp/pytest-synth"),
        output_root=Path("/data/rawdata/output"),
        env=_ALL_FLAGS,
    )
